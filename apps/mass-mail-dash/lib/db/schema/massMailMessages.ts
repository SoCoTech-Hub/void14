...
export const massMailMessages = pgTable('mass_mail_messages', {
  ...
  return {
    massMailListIdIndex: uniqueIndex('mass_mail_list_id_idx').on(massMailMessages.massMailListId),
  }
});
...
export const insertMassMailMessageSchema = createInsertSchema(massMailMessages).omit(timestamps);
export const insertMassMailMessageParams = baseSchema.extend({
...
export const updateMassMailMessageParams = baseSchema.extend({
  publishDate: z.coerce.string().min(1),
  massMailListId: z.coerce.string().min(1)
}).omit({ 
  userId: true
...
export type NewMassMailMessageParams = z.infer<typeof insertMassMailMessageParams>;
export type UpdateMassMailMessageParams = z.infer<typeof updateMassMailMessageParams>;
...
export type CompleteMassMailMessage = Awaited<ReturnType<typeof getMassMailMessages>>["massMailMessages"][number];
