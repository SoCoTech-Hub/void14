...
export async function POST(req: Request) {
  try {
    const validatedData = insertRepositoryInstanceParams.parse(
      await req.json(),
    );
    const { repositoryInstance } =
      await createRepositoryInstance(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateRepositoryInstanceParams.parse(
      await req.json(),
    );
    const validatedParams = repositoryInstanceIdSchema.parse({ id });

    const { repositoryInstance } = await updateRepositoryInstance(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = repositoryInstanceIdSchema.parse({ id });
    const { repositoryInstance } = await deleteRepositoryInstance(
      validatedParams.id,
    );
