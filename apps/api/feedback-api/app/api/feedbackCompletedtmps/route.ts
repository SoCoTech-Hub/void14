...
export async function POST(req: Request) {
  try {
    const validatedData = insertFeedbackCompletedtmpParams.parse(
      await req.json(),
    );
    const { feedbackCompletedtmp } =
      await createFeedbackCompletedtmp(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateFeedbackCompletedtmpParams.parse(
      await req.json(),
    );
    const validatedParams = feedbackCompletedtmpIdSchema.parse({ id });

    const { feedbackCompletedtmp } = await updateFeedbackCompletedtmp(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = feedbackCompletedtmpIdSchema.parse({ id });
    const { feedbackCompletedtmp } = await deleteFeedbackCompletedtmp(
      validatedParams.id,
    );
