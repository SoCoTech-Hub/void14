...
export const createCompetencyUserCompCourse = async (
  competencyUserCompCourse: NewCompetencyUserCompCourseParams,
) => {
  const { session } = await getUserAuth();
  const newCompetencyUserCompCourse =
    insertCompetencyUserCompCourseSchema.parse({
      ...competencyUserCompCourse,
      userId: session?.user.id!,
    });
  try {
    const [c] = await db
      .insert(competencyUserCompCourses)
      .values(newCompetencyUserCompCourse)
      .returning();
...
export const updateCompetencyUserCompCourse = async (
  id: CompetencyUserCompCourseId,
  competencyUserCompCourse: UpdateCompetencyUserCompCourseParams,
) => {
  const { session } = await getUserAuth();
  const { id: competencyUserCompCourseId } =
    competencyUserCompCourseIdSchema.parse({ id });
  const newCompetencyUserCompCourse =
    updateCompetencyUserCompCourseSchema.parse({
      ...competencyUserCompCourse,
      userId: session?.user.id!,
    });
  try {
...
export const deleteCompetencyUserCompCourse = async (
  id: CompetencyUserCompCourseId,
) => {
  const { session } = await getUserAuth();
  const { id: competencyUserCompCourseId } =
  ...
  try {
    const [c] = await db
      .delete(competencyUserCompCourses)
      .where(
        and(
          eq(competencyUserCompCourses.id, competencyUserCompCourseId!),
          eq(competencyUserCompCourses.userId, session?.user.id!),
