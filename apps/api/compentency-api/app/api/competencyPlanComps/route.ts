...
export async function POST(req: Request) {
  try {
    const validatedData = insertCompetencyPlanCompParams.parse(
      await req.json(),
    );
    const { competencyPlanComp } =
      await createCompetencyPlanComp(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateCompetencyPlanCompParams.parse(
      await req.json(),
    );
