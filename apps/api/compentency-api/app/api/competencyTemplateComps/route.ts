...
export async function POST(req: Request) {
	try {
		const validatedData = insertCompetencyTemplateCompParams.parse(
			await req.json()
		)
		const { competencyTemplateComp } =
			await createCompetencyTemplateComp(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateCompetencyTemplateCompParams.parse(
			await req.json()
		)
		const validatedParams = competencyTemplateCompIdSchema.parse({ id })

		const { competencyTemplateComp } = await updateCompetencyTemplateComp(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = competencyTemplateCompIdSchema.parse({ id })
		const { competencyTemplateComp } = await deleteCompetencyTemplateComp(
			validatedParams.id
		)
