...
export async function POST(req: Request) {
  try {
    const validatedData = insertCompetencyUserEvidenceCompParams.parse(
      await req.json(),
    );
    const { competencyUserEvidenceComp } =
      await createCompetencyUserEvidenceComp(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateCompetencyUserEvidenceCompParams.parse(
      await req.json(),
    );
    const validatedParams = competencyUserEvidenceCompIdSchema.parse({ id });

    const { competencyUserEvidenceComp } =
      await updateCompetencyUserEvidenceComp(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = competencyUserEvidenceCompIdSchema.parse({ id });
    const { competencyUserEvidenceComp } =
      await deleteCompetencyUserEvidenceComp(validatedParams.id);
