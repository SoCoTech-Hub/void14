...
export async function POST(req: Request) {
  try {
    const validatedData = insertCompetencyUserCompParams.parse(
      await req.json(),
    );
    const { competencyUserComp } =
      await createCompetencyUserComp(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateCompetencyUserCompParams.parse(
      await req.json(),
    );
