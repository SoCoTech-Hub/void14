...
export async function POST(req: Request) {
  try {
    const validatedData = insertCompetencyCourseCompParams.parse(
      await req.json(),
    );
    const { competencyCourseComp } =
      await createCompetencyCourseComp(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateCompetencyCourseCompParams.parse(
      await req.json(),
    );
    const validatedParams = competencyCourseCompIdSchema.parse({ id });

    const { competencyCourseComp } = await updateCompetencyCourseComp(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = competencyCourseCompIdSchema.parse({ id });
    const { competencyCourseComp } = await deleteCompetencyCourseComp(
      validatedParams.id,
    );
