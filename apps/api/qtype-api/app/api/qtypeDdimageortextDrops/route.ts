...
export async function POST(req: Request) {
  try {
    const validatedData = insertQtypeDdimageortextDropParams.parse(
      await req.json(),
    );
    const { qtypeDdimageortextDrop } =
      await createQtypeDdimageortextDrop(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateQtypeDdimageortextDropParams.parse(
      await req.json(),
    );
    const validatedParams = qtypeDdimageortextDropIdSchema.parse({ id });

    const { qtypeDdimageortextDrop } = await updateQtypeDdimageortextDrop(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = qtypeDdimageortextDropIdSchema.parse({ id });
    const { qtypeDdimageortextDrop } = await deleteQtypeDdimageortextDrop(
      validatedParams.id,
    );
