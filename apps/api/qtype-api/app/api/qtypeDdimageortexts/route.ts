...
export async function POST(req: Request) {
  try {
    const validatedData = insertQtypeDdimageortextParams.parse(
      await req.json(),
    );
    const { qtypeDdimageortext } =
      await createQtypeDdimageortext(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateQtypeDdimageortextParams.parse(
      await req.json(),
    );
