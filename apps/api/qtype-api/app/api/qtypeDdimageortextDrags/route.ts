...
export async function POST(req: Request) {
  try {
    const validatedData = insertQtypeDdimageortextDragParams.parse(
      await req.json(),
    );
    const { qtypeDdimageortextDrag } =
      await createQtypeDdimageortextDrag(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateQtypeDdimageortextDragParams.parse(
      await req.json(),
    );
    const validatedParams = qtypeDdimageortextDragIdSchema.parse({ id });

    const { qtypeDdimageortextDrag } = await updateQtypeDdimageortextDrag(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = qtypeDdimageortextDragIdSchema.parse({ id });
    const { qtypeDdimageortextDrag } = await deleteQtypeDdimageortextDrag(
      validatedParams.id,
    );
