...
export async function POST(req: Request) {
  try {
    const validatedData = insertNotificationResponseParams.parse(
      await req.json(),
    );
    const { notificationResponse } =
      await createNotificationResponse(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateNotificationResponseParams.parse(
      await req.json(),
    );
    const validatedParams = notificationResponseIdSchema.parse({ id });

    const { notificationResponse } = await updateNotificationResponse(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = notificationResponseIdSchema.parse({ id });
    const { notificationResponse } = await deleteNotificationResponse(
      validatedParams.id,
    );
