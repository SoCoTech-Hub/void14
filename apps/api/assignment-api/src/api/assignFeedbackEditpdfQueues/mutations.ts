...
export const createAssignFeedbackEditpdfQueue = async (
  ...
  const newAssignFeedbackEditpdfQueue =
    insertAssignFeedbackEditpdfQueueSchema.parse(assignFeedbackEditpdfQueue);
  try {
    const [a] = await db
      .insert(assignFeedbackEditpdfQueues)
      .values(newAssignFeedbackEditpdfQueue)
      .returning();
...
export const updateAssignFeedbackEditpdfQueue = async (
  id: AssignFeedbackEditpdfQueueId,
  assignFeedbackEditpdfQueue: UpdateAssignFeedbackEditpdfQueueParams,
) => {
  const { id: assignFeedbackEditpdfQueueId } =
    assignFeedbackEditpdfQueueIdSchema.parse({ id });
  const newAssignFeedbackEditpdfQueue =
    updateAssignFeedbackEditpdfQueueSchema.parse(assignFeedbackEditpdfQueue);
  try {
    const [a] = await db
...
export const deleteAssignFeedbackEditpdfQueue = async (
  id: AssignFeedbackEditpdfQueueId,
) => {
  const { id: assignFeedbackEditpdfQueueId } =
    assignFeedbackEditpdfQueueIdSchema.parse({ id });
  try {
    const [a] = await db
      .delete(assignFeedbackEditpdfQueues)
      .where(eq(assignFeedbackEditpdfQueues.id, assignFeedbackEditpdfQueueId!))
      .returning();
    return { assignFeedbackEditpdfQueue: a };
  } catch (err) {
