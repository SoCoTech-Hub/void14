...
export async function POST(req: Request) {
  try {
    const validatedData = insertAssignFeedbackCommentParams.parse(
      await req.json(),
    );
    const { assignFeedbackComment } =
      await createAssignFeedbackComment(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateAssignFeedbackCommentParams.parse(
      await req.json(),
    );
    const validatedParams = assignFeedbackCommentIdSchema.parse({ id });

    const { assignFeedbackComment } = await updateAssignFeedbackComment(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = assignFeedbackCommentIdSchema.parse({ id });
    const { assignFeedbackComment } = await deleteAssignFeedbackComment(
      validatedParams.id,
    );
