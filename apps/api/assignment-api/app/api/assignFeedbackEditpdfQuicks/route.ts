...
export async function POST(req: Request) {
  try {
    const validatedData = insertAssignFeedbackEditpdfQuickParams.parse(
      await req.json(),
    );
    const { assignFeedbackEditpdfQuick } =
      await createAssignFeedbackEditpdfQuick(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateAssignFeedbackEditpdfQuickParams.parse(
      await req.json(),
    );
    const validatedParams = assignFeedbackEditpdfQuickIdSchema.parse({ id });

    const { assignFeedbackEditpdfQuick } =
      await updateAssignFeedbackEditpdfQuick(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = assignFeedbackEditpdfQuickIdSchema.parse({ id });
    const { assignFeedbackEditpdfQuick } =
      await deleteAssignFeedbackEditpdfQuick(validatedParams.id);
