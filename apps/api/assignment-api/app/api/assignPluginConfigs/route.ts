...
export async function POST(req: Request) {
  try {
    const validatedData = insertAssignPluginConfigParams.parse(
      await req.json(),
    );
    const { assignPluginConfig } =
      await createAssignPluginConfig(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateAssignPluginConfigParams.parse(
      await req.json(),
    );
    const validatedParams = assignPluginConfigIdSchema.parse({ id });

    const { assignPluginConfig } = await updateAssignPluginConfig(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = assignPluginConfigIdSchema.parse({ id });
    const { assignPluginConfig } = await deleteAssignPluginConfig(
      validatedParams.id,
    );
