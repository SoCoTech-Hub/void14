...
export async function POST(req: Request) {
  try {
    const validatedData = insertAssignFeedbackFileParams.parse(
      await req.json(),
    );
    const { assignFeedbackFile } =
      await createAssignFeedbackFile(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateAssignFeedbackFileParams.parse(
      await req.json(),
    );
    const validatedParams = assignFeedbackFileIdSchema.parse({ id });

    const { assignFeedbackFile } = await updateAssignFeedbackFile(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = assignFeedbackFileIdSchema.parse({ id });
    const { assignFeedbackFile } = await deleteAssignFeedbackFile(
      validatedParams.id,
    );
