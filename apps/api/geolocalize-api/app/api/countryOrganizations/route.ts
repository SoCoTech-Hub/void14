...
export async function POST(req: Request) {
  try {
    const validatedData = insertCountryOrganizationParams.parse(
      await req.json(),
    );
    const { countryOrganization } =
      await createCountryOrganization(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateCountryOrganizationParams.parse(
      await req.json(),
    );
    const validatedParams = countryOrganizationIdSchema.parse({ id });

    const { countryOrganization } = await updateCountryOrganization(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = countryOrganizationIdSchema.parse({ id });
    const { countryOrganization } = await deleteCountryOrganization(
      validatedParams.id,
    );
