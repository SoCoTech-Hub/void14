...
export async function POST(req: Request) {
  try {
    const validatedData = insertProvinceOrganizationParams.parse(
      await req.json(),
    );
    const { provinceOrganization } =
      await createProvinceOrganization(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateProvinceOrganizationParams.parse(
      await req.json(),
    );
    const validatedParams = provinceOrganizationIdSchema.parse({ id });

    const { provinceOrganization } = await updateProvinceOrganization(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = provinceOrganizationIdSchema.parse({ id });
    const { provinceOrganization } = await deleteProvinceOrganization(
      validatedParams.id,
    );
