...
export const createRoleContextLevel = async (
  ...
  const newRoleContextLevel =
    insertRoleContextLevelSchema.parse(roleContextLevel);
  try {
    const [r] = await db
      .insert(roleContextLevels)
      .values(newRoleContextLevel)
      .returning();
...
export const updateRoleContextLevel = async (
  id: RoleContextLevelId,
  roleContextLevel: UpdateRoleContextLevelParams,
) => {
  const { id: roleContextLevelId } = roleContextLevelIdSchema.parse({ id });
  const newRoleContextLevel =
    updateRoleContextLevelSchema.parse(roleContextLevel);
  try {
    const [r] = await db
