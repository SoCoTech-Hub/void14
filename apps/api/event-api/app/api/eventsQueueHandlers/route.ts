...
export async function POST(req: Request) {
  try {
    const validatedData = insertEventsQueueHandlerParams.parse(
      await req.json(),
    );
    const { eventsQueueHandler } =
      await createEventsQueueHandler(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateEventsQueueHandlerParams.parse(
      await req.json(),
    );
    const validatedParams = eventsQueueHandlerIdSchema.parse({ id });

    const { eventsQueueHandler } = await updateEventsQueueHandler(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = eventsQueueHandlerIdSchema.parse({ id });
    const { eventsQueueHandler } = await deleteEventsQueueHandler(
      validatedParams.id,
    );
