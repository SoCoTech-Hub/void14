...
export const mnetServiceEnrolEnrolments = pgTable(
  ...
  (mnetServiceEnrolEnrolments) => {
    return {
      mnetHostIdIndex: uniqueIndex(
        "mnet_service_enrol_enrolments_mnet_host_id_idx",
      ).on(mnetServiceEnrolEnrolments.mnetHostId),
    };
  },
);
...
export type CompleteMnetServiceEnrolEnrolment = Awaited<
  ReturnType<typeof getMnetServiceEnrolEnrolments>
>["mnetServiceEnrolEnrolments"][number];
