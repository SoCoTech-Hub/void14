...
export async function POST(req: Request) {
  try {
    const validatedData = insertMnetServiceEnrolCourseParams.parse(
      await req.json(),
    );
    const { mnetServiceEnrolCourse } =
      await createMnetServiceEnrolCourse(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateMnetServiceEnrolCourseParams.parse(
      await req.json(),
    );
    const validatedParams = mnetServiceEnrolCourseIdSchema.parse({ id });

    const { mnetServiceEnrolCourse } = await updateMnetServiceEnrolCourse(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = mnetServiceEnrolCourseIdSchema.parse({ id });
    const { mnetServiceEnrolCourse } = await deleteMnetServiceEnrolCourse(
      validatedParams.id,
    );
