...
export async function POST(req: Request) {
  try {
    const validatedData = insertH5pactivityAttemptParams.parse(
      await req.json(),
    );
    const { h5pactivityAttempt } =
      await createH5pactivityAttempt(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateH5pactivityAttemptParams.parse(
      await req.json(),
    );
    const validatedParams = h5pactivityAttemptIdSchema.parse({ id });

    const { h5pactivityAttempt } = await updateH5pactivityAttempt(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = h5pactivityAttemptIdSchema.parse({ id });
    const { h5pactivityAttempt } = await deleteH5pactivityAttempt(
      validatedParams.id,
    );
