...
export async function POST(req: Request) {
  try {
    const validatedData = insertToolMonitorHistoryParams.parse(
      await req.json(),
    );
    const { toolMonitorHistory } =
      await createToolMonitorHistory(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateToolMonitorHistoryParams.parse(
      await req.json(),
    );
    const validatedParams = toolMonitorHistoryIdSchema.parse({ id });

    const { toolMonitorHistory } = await updateToolMonitorHistory(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = toolMonitorHistoryIdSchema.parse({ id });
    const { toolMonitorHistory } = await deleteToolMonitorHistory(
      validatedParams.id,
    );
