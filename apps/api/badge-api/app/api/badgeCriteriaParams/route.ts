...
export async function POST(req: Request) {
  try {
    const validatedData = insertBadgeCriteriaParamParams.parse(
      await req.json(),
    );
    const { badgeCriteriaParam } =
      await createBadgeCriteriaParam(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateBadgeCriteriaParamParams.parse(
      await req.json(),
    );
    const validatedParams = badgeCriteriaParamIdSchema.parse({ id });

    const { badgeCriteriaParam } = await updateBadgeCriteriaParam(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = badgeCriteriaParamIdSchema.parse({ id });
    const { badgeCriteriaParam } = await deleteBadgeCriteriaParam(
      validatedParams.id,
    );
