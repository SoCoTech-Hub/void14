...
export async function POST(req: Request) {
	try {
		const validatedData = insertBadgeExternalBackpackParams.parse(
			await req.json()
		)
		const { badgeExternalBackpack } =
			await createBadgeExternalBackpack(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateBadgeExternalBackpackParams.parse(
			await req.json()
		)
		const validatedParams = badgeExternalBackpackIdSchema.parse({ id })

		const { badgeExternalBackpack } = await updateBadgeExternalBackpack(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = badgeExternalBackpackIdSchema.parse({ id })
		const { badgeExternalBackpack } = await deleteBadgeExternalBackpack(
			validatedParams.id
		)
