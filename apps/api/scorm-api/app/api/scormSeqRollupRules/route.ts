...
export async function POST(req: Request) {
  try {
    const validatedData = insertScormSeqRollupRuleParams.parse(
      await req.json(),
    );
    const { scormSeqRollupRule } =
      await createScormSeqRollupRule(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateScormSeqRollupRuleParams.parse(
      await req.json(),
    );
    const validatedParams = scormSeqRollupRuleIdSchema.parse({ id });

    const { scormSeqRollupRule } = await updateScormSeqRollupRule(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = scormSeqRollupRuleIdSchema.parse({ id });
    const { scormSeqRollupRule } = await deleteScormSeqRollupRule(
      validatedParams.id,
    );
