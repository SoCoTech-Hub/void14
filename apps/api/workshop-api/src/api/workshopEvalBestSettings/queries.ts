import type { WorkshopEvalBestSettingId } from "@soco/workshop-db/schema/workshopEvalBestSettings";
...
export const getWorkshopEvalBestSettings = async () => {
  const rows = await db
    .select({
      workshopEvalBestSetting: workshopEvalBestSettings,
      workshop: workshops,
    })
    .from(workshopEvalBestSettings)
    .leftJoin(workshops, eq(workshopEvalBestSettings.workshopId, workshops.id));
  const w = rows.map((r) => ({
    ...r.workshopEvalBestSetting,
    workshop: r.workshop,
  }));
  return { workshopEvalBestSettings: w };
};

export const getWorkshopEvalBestSettingById = async (
