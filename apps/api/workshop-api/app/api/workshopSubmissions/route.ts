...
export async function POST(req: Request) {
  try {
    const validatedData = insertWorkshopSubmissionParams.parse(
      await req.json(),
    );
    const { workshopSubmission } =
      await createWorkshopSubmission(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateWorkshopSubmissionParams.parse(
      await req.json(),
    );
    const validatedParams = workshopSubmissionIdSchema.parse({ id });

    const { workshopSubmission } = await updateWorkshopSubmission(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = workshopSubmissionIdSchema.parse({ id });
    const { workshopSubmission } = await deleteWorkshopSubmission(
      validatedParams.id,
    );
