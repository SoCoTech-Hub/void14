...
export async function POST(req: Request) {
  try {
    const validatedData = insertWorkshopFormCommentParams.parse(
      await req.json(),
    );
    const { workshopFormComment } =
      await createWorkshopFormComment(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateWorkshopFormCommentParams.parse(
      await req.json(),
    );
    const validatedParams = workshopFormCommentIdSchema.parse({ id });

    const { workshopFormComment } = await updateWorkshopFormComment(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = workshopFormCommentIdSchema.parse({ id });
    const { workshopFormComment } = await deleteWorkshopFormComment(
      validatedParams.id,
    );
