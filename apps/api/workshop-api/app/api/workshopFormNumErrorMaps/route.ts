...
export async function POST(req: Request) {
  try {
    const validatedData = insertWorkshopFormNumErrorMapParams.parse(
      await req.json(),
    );
    const { workshopFormNumErrorMap } =
      await createWorkshopFormNumErrorMap(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateWorkshopFormNumErrorMapParams.parse(
      await req.json(),
    );
    const validatedParams = workshopFormNumErrorMapIdSchema.parse({ id });

    const { workshopFormNumErrorMap } = await updateWorkshopFormNumErrorMap(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = workshopFormNumErrorMapIdSchema.parse({ id });
    const { workshopFormNumErrorMap } = await deleteWorkshopFormNumErrorMap(
      validatedParams.id,
    );
