...
export async function POST(req: Request) {
  try {
    const validatedData = insertWorkshopFormRubricParams.parse(
      await req.json(),
    );
    const { workshopFormRubric } =
      await createWorkshopFormRubric(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateWorkshopFormRubricParams.parse(
      await req.json(),
    );
    const validatedParams = workshopFormRubricIdSchema.parse({ id });

    const { workshopFormRubric } = await updateWorkshopFormRubric(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = workshopFormRubricIdSchema.parse({ id });
    const { workshopFormRubric } = await deleteWorkshopFormRubric(
      validatedParams.id,
    );
