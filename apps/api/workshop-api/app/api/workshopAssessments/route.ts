...
export async function POST(req: Request) {
  try {
    const validatedData = insertWorkshopAssessmentParams.parse(
      await req.json(),
    );
    const { workshopAssessment } =
      await createWorkshopAssessment(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateWorkshopAssessmentParams.parse(
      await req.json(),
    );
    const validatedParams = workshopAssessmentIdSchema.parse({ id });

    const { workshopAssessment } = await updateWorkshopAssessment(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = workshopAssessmentIdSchema.parse({ id });
    const { workshopAssessment } = await deleteWorkshopAssessment(
      validatedParams.id,
    );
