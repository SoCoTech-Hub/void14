...
export async function POST(req: Request) {
  try {
    const validatedData = insertToolRecyclebinCourseParams.parse(
      await req.json(),
    );
    const { toolRecyclebinCourse } =
      await createToolRecyclebinCourse(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateToolRecyclebinCourseParams.parse(
      await req.json(),
    );
    const validatedParams = toolRecyclebinCourseIdSchema.parse({ id });

    const { toolRecyclebinCourse } = await updateToolRecyclebinCourse(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = toolRecyclebinCourseIdSchema.parse({ id });
    const { toolRecyclebinCourse } = await deleteToolRecyclebinCourse(
      validatedParams.id,
    );
