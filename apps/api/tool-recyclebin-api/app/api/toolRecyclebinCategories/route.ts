...
export async function POST(req: Request) {
  try {
    const validatedData = insertToolRecyclebinCategoryParams.parse(
      await req.json(),
    );
    const { toolRecyclebinCategory } =
      await createToolRecyclebinCategory(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateToolRecyclebinCategoryParams.parse(
      await req.json(),
    );
    const validatedParams = toolRecyclebinCategoryIdSchema.parse({ id });

    const { toolRecyclebinCategory } = await updateToolRecyclebinCategory(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = toolRecyclebinCategoryIdSchema.parse({ id });
    const { toolRecyclebinCategory } = await deleteToolRecyclebinCategory(
      validatedParams.id,
    );
