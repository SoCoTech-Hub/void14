...
export async function POST(req: Request) {
  try {
    const validatedData = insertReportbuilderFilterParams.parse(
      await req.json(),
    );
    const { reportbuilderFilter } =
      await createReportbuilderFilter(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateReportbuilderFilterParams.parse(
      await req.json(),
    );
    const validatedParams = reportbuilderFilterIdSchema.parse({ id });

    const { reportbuilderFilter } = await updateReportbuilderFilter(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = reportbuilderFilterIdSchema.parse({ id });
    const { reportbuilderFilter } = await deleteReportbuilderFilter(
      validatedParams.id,
    );
