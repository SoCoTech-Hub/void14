...
export async function POST(req: Request) {
  try {
    const validatedData = insertGradeGradesHistoryParams.parse(
      await req.json(),
    );
    const { gradeGradesHistory } =
      await createGradeGradesHistory(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateGradeGradesHistoryParams.parse(
      await req.json(),
    );
    const validatedParams = gradeGradesHistoryIdSchema.parse({ id });

    const { gradeGradesHistory } = await updateGradeGradesHistory(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = gradeGradesHistoryIdSchema.parse({ id });
    const { gradeGradesHistory } = await deleteGradeGradesHistory(
      validatedParams.id,
    );
