...
export async function POST(req: Request) {
  try {
    const validatedData = insertGradeCategoriesHistoryParams.parse(
      await req.json(),
    );
    const { gradeCategoriesHistory } =
      await createGradeCategoriesHistory(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateGradeCategoriesHistoryParams.parse(
      await req.json(),
    );
    const validatedParams = gradeCategoriesHistoryIdSchema.parse({ id });

    const { gradeCategoriesHistory } = await updateGradeCategoriesHistory(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = gradeCategoriesHistoryIdSchema.parse({ id });
    const { gradeCategoriesHistory } = await deleteGradeCategoriesHistory(
      validatedParams.id,
    );
