...
export async function POST(req: Request) {
  try {
    const validatedData = insertGradingformRubricLevelParams.parse(
      await req.json(),
    );
    const { gradingformRubricLevel } =
      await createGradingformRubricLevel(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateGradingformRubricLevelParams.parse(
      await req.json(),
    );
    const validatedParams = gradingformRubricLevelIdSchema.parse({ id });

    const { gradingformRubricLevel } = await updateGradingformRubricLevel(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = gradingformRubricLevelIdSchema.parse({ id });
    const { gradingformRubricLevel } = await deleteGradingformRubricLevel(
      validatedParams.id,
    );
