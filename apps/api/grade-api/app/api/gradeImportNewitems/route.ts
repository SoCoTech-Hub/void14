...
export async function POST(req: Request) {
  try {
    const validatedData = insertGradeImportNewitemParams.parse(
      await req.json(),
    );
    const { gradeImportNewitem } =
      await createGradeImportNewitem(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateGradeImportNewitemParams.parse(
      await req.json(),
    );
