  ...
  (gradingformGuideCriteria) => {
    return {
      sortOrderIndex: uniqueIndex(
        "gradingform_guide_criteria_sort_order_idx",
      ).on(gradingformGuideCriteria.sortOrder),
    };
  },
);
...
export type CompleteGradingformGuideCriterion = Awaited<
  ReturnType<typeof getGradingformGuideCriteria>
>["gradingformGuideCriteria"][number];
