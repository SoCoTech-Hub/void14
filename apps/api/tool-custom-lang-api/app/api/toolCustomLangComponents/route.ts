...
export async function POST(req: Request) {
  try {
    const validatedData = insertToolCustomLangComponentParams.parse(
      await req.json(),
    );
    const { toolCustomLangComponent } =
      await createToolCustomLangComponent(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateToolCustomLangComponentParams.parse(
      await req.json(),
    );
    const validatedParams = toolCustomLangComponentIdSchema.parse({ id });

    const { toolCustomLangComponent } = await updateToolCustomLangComponent(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = toolCustomLangComponentIdSchema.parse({ id });
    const { toolCustomLangComponent } = await deleteToolCustomLangComponent(
      validatedParams.id,
    );
