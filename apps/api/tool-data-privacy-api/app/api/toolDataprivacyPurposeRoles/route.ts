...
export async function POST(req: Request) {
  try {
    const validatedData = insertToolDataprivacyPurposeRoleParams.parse(
      await req.json(),
    );
    const { toolDataprivacyPurposeRole } =
      await createToolDataprivacyPurposeRole(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateToolDataprivacyPurposeRoleParams.parse(
      await req.json(),
    );
    const validatedParams = toolDataprivacyPurposeRoleIdSchema.parse({ id });

    const { toolDataprivacyPurposeRole } =
      await updateToolDataprivacyPurposeRole(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = toolDataprivacyPurposeRoleIdSchema.parse({ id });
    const { toolDataprivacyPurposeRole } =
      await deleteToolDataprivacyPurposeRole(validatedParams.id);
