...
export async function POST(req: Request) {
  try {
    const validatedData = insertToolDataprivacyRequestParams.parse(
      await req.json(),
    );
    const { toolDataprivacyRequest } =
      await createToolDataprivacyRequest(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateToolDataprivacyRequestParams.parse(
      await req.json(),
    );
    const validatedParams = toolDataprivacyRequestIdSchema.parse({ id });

    const { toolDataprivacyRequest } = await updateToolDataprivacyRequest(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = toolDataprivacyRequestIdSchema.parse({ id });
    const { toolDataprivacyRequest } = await deleteToolDataprivacyRequest(
      validatedParams.id,
    );
