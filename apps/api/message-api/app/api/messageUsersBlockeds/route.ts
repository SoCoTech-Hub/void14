...
export async function POST(req: Request) {
  try {
    const validatedData = insertMessageUsersBlockedParams.parse(
      await req.json(),
    );
    const { messageUsersBlocked } =
      await createMessageUsersBlocked(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateMessageUsersBlockedParams.parse(
      await req.json(),
    );
    const validatedParams = messageUsersBlockedIdSchema.parse({ id });

    const { messageUsersBlocked } = await updateMessageUsersBlocked(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = messageUsersBlockedIdSchema.parse({ id });
    const { messageUsersBlocked } = await deleteMessageUsersBlocked(
      validatedParams.id,
    );
