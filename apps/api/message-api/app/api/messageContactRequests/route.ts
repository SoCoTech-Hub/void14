...
export async function POST(req: Request) {
  try {
    const validatedData = insertMessageContactRequestParams.parse(
      await req.json(),
    );
    const { messageContactRequest } =
      await createMessageContactRequest(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateMessageContactRequestParams.parse(
      await req.json(),
    );
    const validatedParams = messageContactRequestIdSchema.parse({ id });

    const { messageContactRequest } = await updateMessageContactRequest(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = messageContactRequestIdSchema.parse({ id });
    const { messageContactRequest } = await deleteMessageContactRequest(
      validatedParams.id,
    );
