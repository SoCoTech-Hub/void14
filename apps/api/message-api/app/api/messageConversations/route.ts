...
export async function POST(req: Request) {
  try {
    const validatedData = insertMessageConversationParams.parse(
      await req.json(),
    );
    const { messageConversation } =
      await createMessageConversation(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateMessageConversationParams.parse(
      await req.json(),
    );
    const validatedParams = messageConversationIdSchema.parse({ id });

    const { messageConversation } = await updateMessageConversation(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = messageConversationIdSchema.parse({ id });
    const { messageConversation } = await deleteMessageConversation(
      validatedParams.id,
    );
