...
export async function POST(req: Request) {
  try {
    const validatedData = insertMessageConversationActionParams.parse(
      await req.json(),
    );
    const { messageConversationAction } =
      await createMessageConversationAction(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateMessageConversationActionParams.parse(
      await req.json(),
    );
    const validatedParams = messageConversationActionIdSchema.parse({ id });

    const { messageConversationAction } = await updateMessageConversationAction(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = messageConversationActionIdSchema.parse({ id });
    const { messageConversationAction } = await deleteMessageConversationAction(
      validatedParams.id,
    );
