...
export async function POST(req: Request) {
  try {
    const validatedData = insertMessageinboundHandlerParams.parse(
      await req.json(),
    );
    const { messageinboundHandler } =
      await createMessageinboundHandler(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateMessageinboundHandlerParams.parse(
      await req.json(),
    );
    const validatedParams = messageinboundHandlerIdSchema.parse({ id });

    const { messageinboundHandler } = await updateMessageinboundHandler(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = messageinboundHandlerIdSchema.parse({ id });
    const { messageinboundHandler } = await deleteMessageinboundHandler(
      validatedParams.id,
    );
