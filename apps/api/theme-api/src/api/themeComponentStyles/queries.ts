import type { ThemeComponentStyleId } from "@soco/theme-db/schema/themeComponentStyles";
...
export const getThemeComponentStyles = async () => {
  ...
  const rows = await db
    .select({
      themeComponentStyle: themeComponentStyles,
      theme: themes,
      themeComponent: themeComponents,
    })
    .from(themeComponentStyles)
    .leftJoin(themes, eq(themeComponentStyles.themeId, themes.id))
    .leftJoin(
      themeComponents,
      eq(themeComponentStyles.themeComponentId, themeComponents.id),
    )
    .where(eq(themeComponentStyles.userId, session?.user.id!));
  const t = rows.map((r) => ({
    ...r.themeComponentStyle,
    theme: r.theme,
    themeComponent: r.themeComponent,
  }));
