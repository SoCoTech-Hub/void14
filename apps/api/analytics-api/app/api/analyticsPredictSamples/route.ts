...
export async function POST(req: Request) {
  try {
    const validatedData = insertAnalyticsPredictSampleParams.parse(
      await req.json(),
    );
    const { analyticsPredictSample } =
      await createAnalyticsPredictSample(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateAnalyticsPredictSampleParams.parse(
      await req.json(),
    );
    const validatedParams = analyticsPredictSampleIdSchema.parse({ id });

    const { analyticsPredictSample } = await updateAnalyticsPredictSample(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = analyticsPredictSampleIdSchema.parse({ id });
    const { analyticsPredictSample } = await deleteAnalyticsPredictSample(
      validatedParams.id,
    );
