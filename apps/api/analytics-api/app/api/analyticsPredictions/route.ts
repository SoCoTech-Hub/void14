...
export async function POST(req: Request) {
	try {
		const validatedData = insertAnalyticsPredictionParams.parse(
			await req.json()
		)
		const { analyticsPrediction } =
			await createAnalyticsPrediction(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateAnalyticsPredictionParams.parse(
			await req.json()
		)
		const validatedParams = analyticsPredictionIdSchema.parse({ id })

		const { analyticsPrediction } = await updateAnalyticsPrediction(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = analyticsPredictionIdSchema.parse({ id })
		const { analyticsPrediction } = await deleteAnalyticsPrediction(
			validatedParams.id
		)
