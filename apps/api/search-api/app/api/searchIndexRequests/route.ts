...
export async function POST(req: Request) {
  try {
    const validatedData = insertSearchIndexRequestParams.parse(
      await req.json(),
    );
    const { searchIndexRequest } =
      await createSearchIndexRequest(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateSearchIndexRequestParams.parse(
      await req.json(),
    );
