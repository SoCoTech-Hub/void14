...
export async function POST(req: Request) {
  try {
    const validatedData = insertSearchSimpledbIndexParams.parse(
      await req.json(),
    );
    const { searchSimpledbIndex } =
      await createSearchSimpledbIndex(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateSearchSimpledbIndexParams.parse(
      await req.json(),
    );
    const validatedParams = searchSimpledbIndexIdSchema.parse({ id });

    const { searchSimpledbIndex } = await updateSearchSimpledbIndex(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = searchSimpledbIndexIdSchema.parse({ id });
    const { searchSimpledbIndex } = await deleteSearchSimpledbIndex(
      validatedParams.id,
    );
