...
export async function POST(req: Request) {
  try {
    const validatedData = insertApplicationResponseParams.parse(
      await req.json(),
    );
    const { applicationResponse } =
      await createApplicationResponse(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateApplicationResponseParams.parse(
      await req.json(),
    );
    const validatedParams = applicationResponseIdSchema.parse({ id });

    const { applicationResponse } = await updateApplicationResponse(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = applicationResponseIdSchema.parse({ id });
    const { applicationResponse } = await deleteApplicationResponse(
      validatedParams.id,
    );
