...
export async function POST(req: Request) {
  try {
    const validatedData = insertApplicationCategoryParams.parse(
      await req.json(),
    );
    const { applicationCategory } =
      await createApplicationCategory(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateApplicationCategoryParams.parse(
      await req.json(),
    );
    const validatedParams = applicationCategoryIdSchema.parse({ id });

    const { applicationCategory } = await updateApplicationCategory(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = applicationCategoryIdSchema.parse({ id });
    const { applicationCategory } = await deleteApplicationCategory(
      validatedParams.id,
    );
