...
export const createCourseFormatOption = async (
  ...
  const newCourseFormatOption =
    insertCourseFormatOptionSchema.parse(courseFormatOption);
  try {
    const [c] = await db
      .insert(courseFormatOptions)
      .values(newCourseFormatOption)
      .returning();
...
export const updateCourseFormatOption = async (
  id: CourseFormatOptionId,
  courseFormatOption: UpdateCourseFormatOptionParams,
) => {
  const { id: courseFormatOptionId } = courseFormatOptionIdSchema.parse({ id });
  const newCourseFormatOption =
    updateCourseFormatOptionSchema.parse(courseFormatOption);
  try {
    const [c] = await db
