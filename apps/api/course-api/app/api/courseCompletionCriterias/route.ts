...
export async function POST(req: Request) {
	try {
		const validatedData = insertCourseCompletionCriteriaParams.parse(
			await req.json()
		)
		const { courseCompletionCriteria } =
			await createCourseCompletionCriteria(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateCourseCompletionCriteriaParams.parse(
			await req.json()
		)
		const validatedParams = courseCompletionCriteriaIdSchema.parse({ id })

		const { courseCompletionCriteria } = await updateCourseCompletionCriteria(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = courseCompletionCriteriaIdSchema.parse({ id })
		const { courseCompletionCriteria } = await deleteCourseCompletionCriteria(
			validatedParams.id
		)
