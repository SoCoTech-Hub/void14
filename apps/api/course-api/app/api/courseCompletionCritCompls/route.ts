...
export async function POST(req: Request) {
  try {
    const validatedData = insertCourseCompletionCritComplParams.parse(
      await req.json(),
    );
    const { courseCompletionCritCompl } =
      await createCourseCompletionCritCompl(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateCourseCompletionCritComplParams.parse(
      await req.json(),
    );
    const validatedParams = courseCompletionCritComplIdSchema.parse({ id });

    const { courseCompletionCritCompl } = await updateCourseCompletionCritCompl(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = courseCompletionCritComplIdSchema.parse({ id });
    const { courseCompletionCritCompl } = await deleteCourseCompletionCritCompl(
      validatedParams.id,
    );
