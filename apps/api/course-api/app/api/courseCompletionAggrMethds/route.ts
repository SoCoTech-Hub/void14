...
export async function POST(req: Request) {
  try {
    const validatedData = insertCourseCompletionAggrMethdParams.parse(
      await req.json(),
    );
    const { courseCompletionAggrMethd } =
      await createCourseCompletionAggrMethd(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateCourseCompletionAggrMethdParams.parse(
      await req.json(),
    );
    const validatedParams = courseCompletionAggrMethdIdSchema.parse({ id });

    const { courseCompletionAggrMethd } = await updateCourseCompletionAggrMethd(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = courseCompletionAggrMethdIdSchema.parse({ id });
    const { courseCompletionAggrMethd } = await deleteCourseCompletionAggrMethd(
      validatedParams.id,
    );
