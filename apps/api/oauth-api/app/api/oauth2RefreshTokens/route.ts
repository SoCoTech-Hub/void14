...
export async function POST(req: Request) {
  try {
    const validatedData = insertOauth2RefreshTokenParams.parse(
      await req.json(),
    );
    const { oauth2RefreshToken } =
      await createOauth2RefreshToken(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateOauth2RefreshTokenParams.parse(
      await req.json(),
    );
