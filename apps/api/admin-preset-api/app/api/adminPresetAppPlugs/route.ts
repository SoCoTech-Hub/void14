...
export async function POST(req: Request) {
  try {
    const validatedData = insertAdminPresetAppPlugParams.parse(
      await req.json(),
    );
    const { adminPresetAppPlug } =
      await createAdminPresetAppPlug(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateAdminPresetAppPlugParams.parse(
      await req.json(),
    );
    const validatedParams = adminPresetAppPlugIdSchema.parse({ id });

    const { adminPresetAppPlug } = await updateAdminPresetAppPlug(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = adminPresetAppPlugIdSchema.parse({ id });
    const { adminPresetAppPlug } = await deleteAdminPresetAppPlug(
      validatedParams.id,
    );
