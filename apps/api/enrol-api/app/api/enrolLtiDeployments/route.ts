...
export async function POST(req: Request) {
  try {
    const validatedData = insertEnrolLtiDeploymentParams.parse(
      await req.json(),
    );
    const { enrolLtiDeployment } =
      await createEnrolLtiDeployment(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateEnrolLtiDeploymentParams.parse(
      await req.json(),
    );
    const validatedParams = enrolLtiDeploymentIdSchema.parse({ id });

    const { enrolLtiDeployment } = await updateEnrolLtiDeployment(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = enrolLtiDeploymentIdSchema.parse({ id });
    const { enrolLtiDeployment } = await deleteEnrolLtiDeployment(
      validatedParams.id,
    );
