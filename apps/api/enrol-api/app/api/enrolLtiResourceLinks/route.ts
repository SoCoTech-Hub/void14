...
export async function POST(req: Request) {
  try {
    const validatedData = insertEnrolLtiResourceLinkParams.parse(
      await req.json(),
    );
    const { enrolLtiResourceLink } =
      await createEnrolLtiResourceLink(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateEnrolLtiResourceLinkParams.parse(
      await req.json(),
    );
    const validatedParams = enrolLtiResourceLinkIdSchema.parse({ id });

    const { enrolLtiResourceLink } = await updateEnrolLtiResourceLink(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = enrolLtiResourceLinkIdSchema.parse({ id });
    const { enrolLtiResourceLink } = await deleteEnrolLtiResourceLink(
      validatedParams.id,
    );
