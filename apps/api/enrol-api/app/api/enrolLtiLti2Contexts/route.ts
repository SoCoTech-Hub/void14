...
export async function POST(req: Request) {
  try {
    const validatedData = insertEnrolLtiLti2ContextParams.parse(
      await req.json(),
    );
    const { enrolLtiLti2Context } =
      await createEnrolLtiLti2Context(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateEnrolLtiLti2ContextParams.parse(
      await req.json(),
    );
    const validatedParams = enrolLtiLti2ContextIdSchema.parse({ id });

    const { enrolLtiLti2Context } = await updateEnrolLtiLti2Context(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = enrolLtiLti2ContextIdSchema.parse({ id });
    const { enrolLtiLti2Context } = await deleteEnrolLtiLti2Context(
      validatedParams.id,
    );
