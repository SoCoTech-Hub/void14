...
import { type EnrolLtiLti2NonceId, enrolLtiLti2NonceIdSchema, enrolLtiLti2Nonces } from "@soco/enrol-db/schema/enrolLtiLti2Nonces";
...
export const getEnrolLtiLti2NonceById = async (id: EnrolLtiLti2NonceId) => {
  ...
  const [row] = await db.select().from(enrolLtiLti2Nonces).where(eq(enrolLtiLti2Nonces.id, enrolLtiLti2NonceId));
