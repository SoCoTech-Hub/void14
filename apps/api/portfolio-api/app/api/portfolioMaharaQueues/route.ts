...
export async function POST(req: Request) {
  try {
    const validatedData = insertPortfolioMaharaQueueParams.parse(
      await req.json(),
    );
    const { portfolioMaharaQueue } =
      await createPortfolioMaharaQueue(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updatePortfolioMaharaQueueParams.parse(
      await req.json(),
    );
    const validatedParams = portfolioMaharaQueueIdSchema.parse({ id });

    const { portfolioMaharaQueue } = await updatePortfolioMaharaQueue(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = portfolioMaharaQueueIdSchema.parse({ id });
    const { portfolioMaharaQueue } = await deletePortfolioMaharaQueue(
      validatedParams.id,
    );
