import type { QuestionCalculatedOptionId } from "@soco/question-db/schema/questionCalculatedOptions";
...
export const getQuestionCalculatedOptions = async () => {
  const rows = await db
    .select({
      questionCalculatedOption: questionCalculatedOptions,
      question: questions,
    })
    .from(questionCalculatedOptions)
    .leftJoin(
      questions,
      eq(questionCalculatedOptions.questionId, questions.id),
    );
  const q = rows.map((r) => ({
    ...r.questionCalculatedOption,
    question: r.question,
  }));
  return { questionCalculatedOptions: q };
};

export const getQuestionCalculatedOptionById = async (
