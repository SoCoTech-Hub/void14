...
export async function POST(req: Request) {
	try {
		const validatedData = insertQuestionAttemptStepParams.parse(
			await req.json()
		)
		const { questionAttemptStep } =
			await createQuestionAttemptStep(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateQuestionAttemptStepParams.parse(
			await req.json()
		)
		const validatedParams = questionAttemptStepIdSchema.parse({ id })

		const { questionAttemptStep } = await updateQuestionAttemptStep(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = questionAttemptStepIdSchema.parse({ id })
		const { questionAttemptStep } = await deleteQuestionAttemptStep(
			validatedParams.id
		)
