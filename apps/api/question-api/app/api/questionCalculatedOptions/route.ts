...
export async function POST(req: Request) {
  try {
    const validatedData = insertQuestionCalculatedOptionParams.parse(
      await req.json(),
    );
    const { questionCalculatedOption } =
      await createQuestionCalculatedOption(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateQuestionCalculatedOptionParams.parse(
      await req.json(),
    );
    const validatedParams = questionCalculatedOptionIdSchema.parse({ id });

    const { questionCalculatedOption } = await updateQuestionCalculatedOption(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = questionCalculatedOptionIdSchema.parse({ id });
    const { questionCalculatedOption } = await deleteQuestionCalculatedOption(
      validatedParams.id,
    );
