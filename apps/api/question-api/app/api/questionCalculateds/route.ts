...
export async function POST(req: Request) {
  try {
    const validatedData = insertQuestionCalculatedParams.parse(
      await req.json(),
    );
    const { questionCalculated } =
      await createQuestionCalculated(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateQuestionCalculatedParams.parse(
      await req.json(),
    );
    const validatedParams = questionCalculatedIdSchema.parse({ id });

    const { questionCalculated } = await updateQuestionCalculated(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = questionCalculatedIdSchema.parse({ id });
    const { questionCalculated } = await deleteQuestionCalculated(
      validatedParams.id,
    );
