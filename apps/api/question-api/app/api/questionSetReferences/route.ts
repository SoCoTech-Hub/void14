...
export async function POST(req: Request) {
  try {
    const validatedData = insertQuestionSetReferenceParams.parse(
      await req.json(),
    );
    const { questionSetReference } =
      await createQuestionSetReference(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateQuestionSetReferenceParams.parse(
      await req.json(),
    );
    const validatedParams = questionSetReferenceIdSchema.parse({ id });

    const { questionSetReference } = await updateQuestionSetReference(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = questionSetReferenceIdSchema.parse({ id });
    const { questionSetReference } = await deleteQuestionSetReference(
      validatedParams.id,
    );
