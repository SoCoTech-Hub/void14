...
export const createQuestionUsage = async (
  questionUsage: NewQuestionUsageParams,
) => {
  const newQuestionUsage = insertQuestionUsageSchema.parse(questionUsage);
  try {
    const [q] = await db
      .insert(questionUsages)
      .values(newQuestionUsage)
      .returning();
...
export const updateQuestionUsage = async (
  id: QuestionUsageId,
  questionUsage: UpdateQuestionUsageParams,
) => {
