  ...
  (questionCalculatedOptions) => {
    return {
      questionIdIndex: uniqueIndex(
        "question_calculated_options_question_id_idx",
      ).on(questionCalculatedOptions.questionId),
    };
  },
);
...
export type CompleteQuestionCalculatedOption = Awaited<
  ReturnType<typeof getQuestionCalculatedOptions>
>["questionCalculatedOptions"][number];
