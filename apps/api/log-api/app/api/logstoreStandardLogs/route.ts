...
export async function POST(req: Request) {
  try {
    const validatedData = insertLogstoreStandardLogParams.parse(
      await req.json(),
    );
    const { logstoreStandardLog } =
      await createLogstoreStandardLog(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateLogstoreStandardLogParams.parse(
      await req.json(),
    );
    const validatedParams = logstoreStandardLogIdSchema.parse({ id });

    const { logstoreStandardLog } = await updateLogstoreStandardLog(
      validatedParams.id,
      validatedData,
    );
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = logstoreStandardLogIdSchema.parse({ id });
    const { logstoreStandardLog } = await deleteLogstoreStandardLog(
      validatedParams.id,
    );
