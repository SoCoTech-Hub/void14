...
export const insertBursarySchema = createInsertSchema(bursaries).omit(timestamps);
export const insertBursaryParams = baseSchema.extend({
...
export const updateBursaryParams = baseSchema.extend({
  openDate: z.coerce.string().min(1),
  closeDate: z.coerce.string().min(1)
}).omit({ 
  userId: true
...
export type CompleteBursary = Awaited<ReturnType<typeof getBursaries>>["bursaries"][number];
