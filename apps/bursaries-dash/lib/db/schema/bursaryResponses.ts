...
export const bursaryResponses = pgTable('bursary_responses', {
  ...
  bursaryId: varchar("bursary_id", { length: 256 }).references(() => bursaries.id).notNull(),
...
export const insertBursaryResponseSchema = createInsertSchema(bursaryResponses).omit(timestamps);
export const insertBursaryResponseParams = baseSchema.extend({
...
export const updateBursaryResponseParams = baseSchema.extend({
  bursaryId: z.coerce.string().min(1)
}).omit({ 
  userId: true
});
...
export type NewBursaryResponseParams = z.infer<typeof insertBursaryResponseParams>;
export type UpdateBursaryResponseParams = z.infer<typeof updateBursaryResponseParams>;
...
export type CompleteBursaryResponse = Awaited<ReturnType<typeof getBursaryResponses>>["bursaryResponses"][number];
