...
export async function POST(req: Request) {
  try {
    const validatedData = insertQualificationsResponseParams.parse(await req.json());
    const { qualificationsResponse } = await createQualificationsResponse(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateQualificationsResponseParams.parse(await req.json());
    const validatedParams = qualificationsResponseIdSchema.parse({ id });

    const { qualificationsResponse } = await updateQualificationsResponse(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = qualificationsResponseIdSchema.parse({ id });
    const { qualificationsResponse } = await deleteQualificationsResponse(validatedParams.id);
