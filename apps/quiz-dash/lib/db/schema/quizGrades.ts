...
export const quizGrades = pgTable('quiz_grades', {
  ...
  grade: real("grade"),
  quizeId: varchar("quize_id", { length: 256 }).references(() => quizes.id, { onDelete: "cascade" }).notNull(),
  ...
  return {
    quizeIdIndex: uniqueIndex('quize_id_idx').on(quizGrades.quizeId),
  }
});
...
export const insertQuizGradeSchema = createInsertSchema(quizGrades).omit(timestamps);
export const insertQuizGradeParams = baseSchema.extend({
...
export const updateQuizGradeParams = baseSchema.extend({
  grade: z.coerce.number(),
  quizeId: z.coerce.string().min(1)
}).omit({ 
  userId: true
...
export type CompleteQuizGrade = Awaited<ReturnType<typeof getQuizGrades>>["quizGrades"][number];
