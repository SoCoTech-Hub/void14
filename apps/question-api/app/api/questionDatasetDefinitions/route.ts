...
export async function POST(req: Request) {
	try {
		const validatedData = insertQuestionDatasetDefinitionParams.parse(
			await req.json()
		)
		const { questionDatasetDefinition } =
			await createQuestionDatasetDefinition(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateQuestionDatasetDefinitionParams.parse(
			await req.json()
		)
		const validatedParams = questionDatasetDefinitionIdSchema.parse({ id })

		const { questionDatasetDefinition } = await updateQuestionDatasetDefinition(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = questionDatasetDefinitionIdSchema.parse({ id })
		const { questionDatasetDefinition } = await deleteQuestionDatasetDefinition(
			validatedParams.id
		)
