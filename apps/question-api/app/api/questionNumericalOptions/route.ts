...
export async function POST(req: Request) {
	try {
		const validatedData = insertQuestionNumericalOptionParams.parse(
			await req.json()
		)
		const { questionNumericalOption } =
			await createQuestionNumericalOption(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateQuestionNumericalOptionParams.parse(
			await req.json()
		)
		const validatedParams = questionNumericalOptionIdSchema.parse({ id })

		const { questionNumericalOption } = await updateQuestionNumericalOption(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = questionNumericalOptionIdSchema.parse({ id })
		const { questionNumericalOption } = await deleteQuestionNumericalOption(
			validatedParams.id
		)
