...
import { integer, real, text, varchar, boolean, timestamp, pgTable } from "drizzle-orm/pg-core";
...
export const gradeItemsHistories = pgTable('grade_items_histories', {
  ...
  return {
    sortOrderIndex: uniqueIndex('sort_order_idx').on(gradeItemsHistories.sortOrder),
  }
});
...
export const insertGradeItemsHistorySchema = createInsertSchema(gradeItemsHistories).omit(timestamps);
export const insertGradeItemsHistoryParams = baseSchema.extend({
...
export const updateGradeItemsHistoryParams = baseSchema.extend({
  action: z.coerce.number(),
  aggregationCoef: z.coerce.number(),
  aggregationCoef2: z.coerce.number(),
  decimals: z.coerce.boolean(),
...
export type NewGradeItemsHistory = z.infer<typeof insertGradeItemsHistorySchema>;
export type NewGradeItemsHistoryParams = z.infer<typeof insertGradeItemsHistoryParams>;
export type UpdateGradeItemsHistoryParams = z.infer<typeof updateGradeItemsHistoryParams>;
export type GradeItemsHistoryId = z.infer<typeof gradeItemsHistoryIdSchema>["id"];
    
// this type infers the return from getGradeItemsHistories() - meaning it will include any joins
export type CompleteGradeItemsHistory = Awaited<ReturnType<typeof getGradeItemsHistories>>["gradeItemsHistories"][number];
