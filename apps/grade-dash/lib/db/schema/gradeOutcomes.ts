...
export const insertGradeOutcomeSchema = createInsertSchema(gradeOutcomes).omit(timestamps);
export const insertGradeOutcomeParams = baseSchema.extend({
...
export const updateGradeOutcomeParams = baseSchema.extend({
  descriptionFormat: z.coerce.number()
}).omit({ 
  userId: true
});
...
export type CompleteGradeOutcome = Awaited<ReturnType<typeof getGradeOutcomes>>["gradeOutcomes"][number];
