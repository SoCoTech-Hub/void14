...
import { real, text, varchar, boolean, integer, timestamp, pgTable } from "drizzle-orm/pg-core";
...
export const gradeItems = pgTable('grade_items', {
  ...
  return {
    sortOrderIndex: uniqueIndex('sort_order_idx').on(gradeItems.sortOrder),
  }
});
...
export const insertGradeItemSchema = createInsertSchema(gradeItems).omit(timestamps);
export const insertGradeItemParams = baseSchema.extend({
...
export type CompleteGradeItem = Awaited<ReturnType<typeof getGradeItems>>["gradeItems"][number];
