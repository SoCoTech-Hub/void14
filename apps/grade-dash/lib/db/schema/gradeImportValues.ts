import { text, real, varchar, boolean, pgTable } from "drizzle-orm/pg-core";
import { createInsertSchema, createSelectSchema } from "drizzle-zod";
import { z } from "zod";

import { type getGradeImportValues } from "@/lib/api/gradeImportValues/queries";
...
export const insertGradeImportValueSchema = createInsertSchema(gradeImportValues);
export const insertGradeImportValueParams = baseSchema.extend({
...
export const updateGradeImportValueParams = baseSchema.extend({
  finalGrade: z.coerce.number(),
  importOnlyFeedback: z.coerce.boolean()
}).omit({ 
  userId: true
...
export type NewGradeImportValueParams = z.infer<typeof insertGradeImportValueParams>;
export type UpdateGradeImportValueParams = z.infer<typeof updateGradeImportValueParams>;
...
export type CompleteGradeImportValue = Awaited<ReturnType<typeof getGradeImportValues>>["gradeImportValues"][number];
