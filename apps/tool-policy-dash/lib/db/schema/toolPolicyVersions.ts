...
import { integer, text, varchar, timestamp, pgTable } from "drizzle-orm/pg-core";
...
export const toolPolicyVersions = pgTable('tool_policy_versions', {
  ...
  toolPolicyId: varchar("tool_policy_id", { length: 256 }).references(() => toolPolicies.id, { onDelete: "cascade" }).notNull(),
...
export const insertToolPolicyVersionSchema = createInsertSchema(toolPolicyVersions).omit(timestamps);
export const insertToolPolicyVersionParams = baseSchema.extend({
...
export const updateToolPolicyVersionParams = baseSchema.extend({
  agreementStyle: z.coerce.number(),
  archived: z.coerce.number(),
  audience: z.coerce.number(),
  contentFormat: z.coerce.number(),
...
export type NewToolPolicyVersion = z.infer<typeof insertToolPolicyVersionSchema>;
export type NewToolPolicyVersionParams = z.infer<typeof insertToolPolicyVersionParams>;
export type UpdateToolPolicyVersionParams = z.infer<typeof updateToolPolicyVersionParams>;
export type ToolPolicyVersionId = z.infer<typeof toolPolicyVersionIdSchema>["id"];
    
// this type infers the return from getToolPolicyVersions() - meaning it will include any joins
export type CompleteToolPolicyVersion = Awaited<ReturnType<typeof getToolPolicyVersions>>["toolPolicyVersions"][number];
