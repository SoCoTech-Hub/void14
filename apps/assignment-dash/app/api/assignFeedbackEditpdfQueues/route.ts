...
export async function POST(req: Request) {
  try {
    const validatedData = insertAssignFeedbackEditpdfQueueParams.parse(await req.json());
    const { assignFeedbackEditpdfQueue } = await createAssignFeedbackEditpdfQueue(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateAssignFeedbackEditpdfQueueParams.parse(await req.json());
    const validatedParams = assignFeedbackEditpdfQueueIdSchema.parse({ id });

    const { assignFeedbackEditpdfQueue } = await updateAssignFeedbackEditpdfQueue(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = assignFeedbackEditpdfQueueIdSchema.parse({ id });
    const { assignFeedbackEditpdfQueue } = await deleteAssignFeedbackEditpdfQueue(validatedParams.id);
