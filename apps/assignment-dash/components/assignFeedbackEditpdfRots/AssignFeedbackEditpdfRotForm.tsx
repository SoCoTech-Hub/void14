...
import { AssignFeedbackEditpdfRot, NewAssignFeedbackEditpdfRotParams, insertAssignFeedbackEditpdfRotParams } from "@/lib/db/schema/assignFeedbackEditpdfRots";
...
const AssignFeedbackEditpdfRotForm = ({
  ...
  const handleSubmit = (values: NewAssignFeedbackEditpdfRotParams) => {
    if (editing) {
      updateAssignFeedbackEditpdfRot({ ...values, id: assignFeedbackEditpdfRot.id });
    } else {
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className={"space-y-8"}>
        ...
        {editing ? (
          <Button
            type="button"
            variant={"destructive"}
            onClick={() => deleteAssignFeedbackEditpdfRot({ id: assignFeedbackEditpdfRot.id })}
          >
