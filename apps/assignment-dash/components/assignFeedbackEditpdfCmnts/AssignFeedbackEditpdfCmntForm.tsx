...
import { AssignFeedbackEditpdfCmnt, NewAssignFeedbackEditpdfCmntParams, insertAssignFeedbackEditpdfCmntParams } from "@/lib/db/schema/assignFeedbackEditpdfCmnts";
...
const AssignFeedbackEditpdfCmntForm = ({
  ...
  const handleSubmit = (values: NewAssignFeedbackEditpdfCmntParams) => {
    if (editing) {
      updateAssignFeedbackEditpdfCmnt({ ...values, id: assignFeedbackEditpdfCmnt.id });
    } else {
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className={"space-y-8"}>
        ...
        {editing ? (
          <Button
            type="button"
            variant={"destructive"}
            onClick={() => deleteAssignFeedbackEditpdfCmnt({ id: assignFeedbackEditpdfCmnt.id })}
          >
