...
import { AssignFeedbackEditpdfQueue, NewAssignFeedbackEditpdfQueueParams, insertAssignFeedbackEditpdfQueueParams } from "@/lib/db/schema/assignFeedbackEditpdfQueues";
...
const AssignFeedbackEditpdfQueueForm = ({
  ...
  const handleSubmit = (values: NewAssignFeedbackEditpdfQueueParams) => {
    if (editing) {
      updateAssignFeedbackEditpdfQueue({ ...values, id: assignFeedbackEditpdfQueue.id });
    } else {
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className={"space-y-8"}>
        ...
        {editing ? (
          <Button
            type="button"
            variant={"destructive"}
            onClick={() => deleteAssignFeedbackEditpdfQueue({ id: assignFeedbackEditpdfQueue.id })}
          >
