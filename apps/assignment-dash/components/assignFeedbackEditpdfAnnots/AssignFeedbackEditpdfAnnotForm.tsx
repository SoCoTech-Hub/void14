...
import { AssignFeedbackEditpdfAnnot, NewAssignFeedbackEditpdfAnnotParams, insertAssignFeedbackEditpdfAnnotParams } from "@/lib/db/schema/assignFeedbackEditpdfAnnots";
...
const AssignFeedbackEditpdfAnnotForm = ({
  ...
  const handleSubmit = (values: NewAssignFeedbackEditpdfAnnotParams) => {
    if (editing) {
      updateAssignFeedbackEditpdfAnnot({ ...values, id: assignFeedbackEditpdfAnnot.id });
    } else {
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className={"space-y-8"}>
        ...
        {editing ? (
          <Button
            type="button"
            variant={"destructive"}
            onClick={() => deleteAssignFeedbackEditpdfAnnot({ id: assignFeedbackEditpdfAnnot.id })}
          >
