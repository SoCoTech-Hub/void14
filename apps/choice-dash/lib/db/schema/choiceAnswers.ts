...
export const choiceAnswers = pgTable('choice_answers', {
  ...
  choiceOptionId: varchar("choice_option_id", { length: 256 }).references(() => choiceOptions.id, { onDelete: "cascade" }).notNull(),
  choiceId: varchar("choice_id", { length: 256 }).references(() => choices.id, { onDelete: "cascade" }).notNull(),
...
export const insertChoiceAnswerSchema = createInsertSchema(choiceAnswers).omit(timestamps);
export const insertChoiceAnswerParams = baseSchema.extend({
...
export const updateChoiceAnswerParams = baseSchema.extend({
  choiceOptionId: z.coerce.string().min(1),
  choiceId: z.coerce.string().min(1)
}).omit({ 
  userId: true
...
export type CompleteChoiceAnswer = Awaited<ReturnType<typeof getChoiceAnswers>>["choiceAnswers"][number];
