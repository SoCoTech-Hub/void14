...
export const localizationTranslations = pgTable('localization_translations', {
  ...
  localizationFieldId: varchar("localization_field_id", { length: 256 }).references(() => localizationFields.id, { onDelete: "cascade" }).notNull(),
  localizationLanguageId: varchar("localization_language_id", { length: 256 }).references(() => localizationLanguages.id, { onDelete: "cascade" }).notNull(),
...
export const insertLocalizationTranslationParams = baseSchema.extend({
...
export type NewLocalizationTranslation = z.infer<typeof insertLocalizationTranslationSchema>;
export type NewLocalizationTranslationParams = z.infer<typeof insertLocalizationTranslationParams>;
export type UpdateLocalizationTranslationParams = z.infer<typeof updateLocalizationTranslationParams>;
export type LocalizationTranslationId = z.infer<typeof localizationTranslationIdSchema>["id"];
    
// this type infers the return from getLocalizationTranslations() - meaning it will include any joins
export type CompleteLocalizationTranslation = Awaited<ReturnType<typeof getLocalizationTranslations>>["localizationTranslations"][number];
