...
export const insertSessionParams = baseSchema.extend({
...
export const updateSessionParams = baseSchema.extend({
  state: z.coerce.number()
}).omit({ 
  userId: true
});
...
export type CompleteSession = Awaited<ReturnType<typeof getSessions>>["sessions"][number];
