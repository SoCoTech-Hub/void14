...
import { type getMnetService2rpcs } from "@/lib/api/mnetService2rpcs/queries";

import { nanoid } from "@/lib/utils";


export const mnetService2rpcs = pgTable('mnet_service2rpcs', {
  id: varchar("id", { length: 191 }).primaryKey().$defaultFn(() => nanoid()),
  mnetServiceId: varchar("mnet_service_id", { length: 256 }).references(() => mnetServices.id, { onDelete: "cascade" }).notNull(),
  mnetRpcId: varchar("mnet_rpc_id", { length: 256 }).references(() => mnetRpcs.id, { onDelete: "cascade" }).notNull()
}, (mnetService2rpcs) => {
  return {
    mnetRpcIdIndex: uniqueIndex('mnet_rpc_id_idx').on(mnetService2rpcs.mnetRpcId),
  }
});
...
export const insertMnetService2rpcParams = baseSchema.extend({
...
export type NewMnetService2rpcParams = z.infer<typeof insertMnetService2rpcParams>;
export type UpdateMnetService2rpcParams = z.infer<typeof updateMnetService2rpcParams>;
...
export type CompleteMnetService2rpc = Awaited<ReturnType<typeof getMnetService2rpcs>>["mnetService2rpcs"][number];
