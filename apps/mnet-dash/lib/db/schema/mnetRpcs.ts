import { varchar, boolean, text, pgTable } from "drizzle-orm/pg-core";
...
export const mnetRpcs = pgTable('mnet_rpcs', {
  ...
  return {
    functionNameIndex: uniqueIndex('function_name_idx').on(mnetRpcs.functionName),
  }
});
...
export const insertMnetRpcParams = baseSchema.extend({
...
export type CompleteMnetRpc = Awaited<ReturnType<typeof getMnetRpcs>>["mnetRpcs"][number];
