...
export const mnetSessions = pgTable('mnet_sessions', {
  ...
  return {
    mnetHostIdIndex: uniqueIndex('mnet_host_id_idx').on(mnetSessions.mnetHostId),
  }
});
...
export const insertMnetSessionParams = baseSchema.extend({
...
export const updateMnetSessionParams = baseSchema.extend({
  confirmTimeout: z.coerce.number(),
  expires: z.coerce.number(),
  mnetHostId: z.coerce.string().min(1)
}).omit({ 
...
export type CompleteMnetSession = Awaited<ReturnType<typeof getMnetSessions>>["mnetSessions"][number];
