...
export const insertScaleParams = baseSchema.extend({
...
export const updateScaleParams = baseSchema.extend({
  descriptionFormat: z.coerce.number()
}).omit({ 
  userId: true
});
...
export type CompleteScale = Awaited<ReturnType<typeof getScales>>["scales"][number];
