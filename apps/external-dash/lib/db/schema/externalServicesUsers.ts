...
export const insertExternalServicesUserSchema = createInsertSchema(externalServicesUsers).omit(timestamps);
export const insertExternalServicesUserParams = baseSchema.extend({
...
export const updateExternalServicesUserParams = baseSchema.extend({
  validUntil: z.coerce.string().min(1)
}).omit({ 
  userId: true
});
...
export type NewExternalServicesUser = z.infer<typeof insertExternalServicesUserSchema>;
export type NewExternalServicesUserParams = z.infer<typeof insertExternalServicesUserParams>;
export type UpdateExternalServicesUserParams = z.infer<typeof updateExternalServicesUserParams>;
export type ExternalServicesUserId = z.infer<typeof externalServicesUserIdSchema>["id"];
    
// this type infers the return from getExternalServicesUsers() - meaning it will include any joins
export type CompleteExternalServicesUser = Awaited<ReturnType<typeof getExternalServicesUsers>>["externalServicesUsers"][number];
