...
export const insertExternalTokenSchema = createInsertSchema(externalTokens).omit(timestamps);
export const insertExternalTokenParams = baseSchema.extend({
...
export const updateExternalTokenParams = baseSchema.extend({
  lastAccess: z.coerce.string().min(1),
  tokenType: z.coerce.boolean(),
  validUntil: z.coerce.string().min(1)
}).omit({ 
...
export type UpdateExternalTokenParams = z.infer<typeof updateExternalTokenParams>;
...
export type CompleteExternalToken = Awaited<ReturnType<typeof getExternalTokens>>["externalTokens"][number];
