...
import { varchar, integer, boolean, timestamp, pgTable, uniqueIndex } from "drizzle-orm/pg-core";
...
export const digilibs = pgTable('digilibs', {
  ...
  digilibCategoryId: varchar("digilib_category_id", { length: 256 }).references(() => digilibCategories.id, { onDelete: "cascade" }).notNull(),
  ...
  return {
    digilibCategoryIdIndex: uniqueIndex('digilib_category_id_idx').on(digilibs.digilibCategoryId),
  }
});
...
export const insertDigilibParams = baseSchema.extend({
...
export type CompleteDigilib = Awaited<ReturnType<typeof getDigilibs>>["digilibs"][number];
