...
export async function POST(req: Request) {
	try {
		const validatedData = insertEnrolLtiLti2ResourceLinkParams.parse(
			await req.json()
		)
		const { enrolLtiLti2ResourceLink } =
			await createEnrolLtiLti2ResourceLink(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateEnrolLtiLti2ResourceLinkParams.parse(
			await req.json()
		)
		const validatedParams = enrolLtiLti2ResourceLinkIdSchema.parse({ id })

		const { enrolLtiLti2ResourceLink } = await updateEnrolLtiLti2ResourceLink(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = enrolLtiLti2ResourceLinkIdSchema.parse({ id })
		const { enrolLtiLti2ResourceLink } = await deleteEnrolLtiLti2ResourceLink(
			validatedParams.id
		)
