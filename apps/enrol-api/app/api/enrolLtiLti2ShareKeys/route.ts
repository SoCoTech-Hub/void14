...
export async function POST(req: Request) {
	try {
		const validatedData = insertEnrolLtiLti2ShareKeyParams.parse(
			await req.json()
		)
		const { enrolLtiLti2ShareKey } =
			await createEnrolLtiLti2ShareKey(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateEnrolLtiLti2ShareKeyParams.parse(
			await req.json()
		)
		const validatedParams = enrolLtiLti2ShareKeyIdSchema.parse({ id })

		const { enrolLtiLti2ShareKey } = await updateEnrolLtiLti2ShareKey(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = enrolLtiLti2ShareKeyIdSchema.parse({ id })
		const { enrolLtiLti2ShareKey } = await deleteEnrolLtiLti2ShareKey(
			validatedParams.id
		)
