...
export const insertLessonBranchParams = baseSchema.extend({
...
export const updateLessonBranchParams = baseSchema.extend({
  flag: z.coerce.number(),
  lessonId: z.coerce.string().min(1),
  lessonPageId: z.coerce.string().min(1),
  retry: z.coerce.number(),
...
export type CompleteLessonBranch = Awaited<ReturnType<typeof getLessonBranches>>["lessonBranches"][number];
