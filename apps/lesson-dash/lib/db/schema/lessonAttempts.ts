...
export const lessonAttempts = pgTable('lesson_attempts', {
  ...
  lessonAnswerId: varchar("lesson_answer_id", { length: 256 }).references(() => lessonAnswers.id, { onDelete: "cascade" }).notNull(),
  lessonPageId: varchar("lesson_page_id", { length: 256 }).references(() => lessonPages.id, { onDelete: "cascade" }).notNull(),
...
export const insertLessonAttemptSchema = createInsertSchema(lessonAttempts).omit(timestamps);
export const insertLessonAttemptParams = baseSchema.extend({
...
export const updateLessonAttemptParams = baseSchema.extend({
  lessonAnswerId: z.coerce.string().min(1),
  lessonPageId: z.coerce.string().min(1),
  lessonId: z.coerce.string().min(1),
  correct: z.coerce.boolean(),
...
export type UpdateLessonAttemptParams = z.infer<typeof updateLessonAttemptParams>;
...
export type CompleteLessonAttempt = Awaited<ReturnType<typeof getLessonAttempts>>["lessonAttempts"][number];
