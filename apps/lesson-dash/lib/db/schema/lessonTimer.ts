...
export const insertLessonTimerSchema = createInsertSchema(lessonTimer).omit(timestamps);
export const insertLessonTimerParams = baseSchema.extend({
...
export const updateLessonTimerParams = baseSchema.extend({
  completed: z.coerce.boolean(),
  lessonId: z.coerce.string().min(1),
  lessonTime: z.coerce.number(),
  startTime: z.coerce.string().min(1)
...
export type CompleteLessonTimer = Awaited<ReturnType<typeof getLessonTimer>>["lessonTimer"][number];
