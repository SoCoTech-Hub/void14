...
export const tagInstances = pgTable('tag_instances', {
  ...
  tagId: varchar("tag_id", { length: 256 }).references(() => tags.id).notNull(),
...
export const insertTagInstanceSchema = createInsertSchema(tagInstances).omit(timestamps);
export const insertTagInstanceParams = baseSchema.extend({
...
export const updateTagInstanceParams = baseSchema.extend({
  ordering: z.coerce.number(),
  tagId: z.coerce.string().min(1)
}).omit({ 
  userId: true
...
export type CompleteTagInstance = Awaited<ReturnType<typeof getTagInstances>>["tagInstances"][number];
