...
export const tagAreas = pgTable('tag_areas', {
  ...
  tagCollId: varchar("tag_coll_id", { length: 256 }).references(() => tagColls.id).notNull()
});
...
export const insertTagAreaParams = baseSchema.extend({
...
export type CompleteTagArea = Awaited<ReturnType<typeof getTagAreas>>["tagAreas"][number];
