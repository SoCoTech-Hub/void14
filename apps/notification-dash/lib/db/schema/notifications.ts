...
export const insertNotificationSchema = createInsertSchema(notifications).omit(timestamps);
export const insertNotificationParams = baseSchema.extend({
...
export const updateNotificationParams = baseSchema.extend({
  fullMessageFormat: z.coerce.number(),
  timeRead: z.coerce.string().min(1)
}).omit({ 
  userId: true
