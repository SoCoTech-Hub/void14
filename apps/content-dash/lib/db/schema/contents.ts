...
import { boolean, text, varchar, timestamp, pgTable } from "drizzle-orm/pg-core";
...
export const insertContentSchema = createInsertSchema(contents).omit(timestamps);
export const insertContentParams = baseSchema.extend({
...
export const updateContentParams = baseSchema.extend({
  visibility: z.coerce.boolean()
}).omit({ 
  userId: true
});
...
export type CompleteContent = Awaited<ReturnType<typeof getContents>>["contents"][number];
