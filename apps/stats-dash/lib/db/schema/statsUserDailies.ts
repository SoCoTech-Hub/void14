...
export const insertStatsUserDailyParams = baseSchema.extend({
...
export const updateStatsUserDailyParams = baseSchema.extend({
  timeEnd: z.coerce.string().min(1)
}).omit({ 
  userId: true
});
...
export type NewStatsUserDailyParams = z.infer<typeof insertStatsUserDailyParams>;
export type UpdateStatsUserDailyParams = z.infer<typeof updateStatsUserDailyParams>;
...
export type CompleteStatsUserDaily = Awaited<ReturnType<typeof getStatsUserDailies>>["statsUserDailies"][number];
