...
export const insertStatsUserMonthlySchema = createInsertSchema(statsUserMonthlies);
export const insertStatsUserMonthlyParams = baseSchema.extend({
...
export const updateStatsUserMonthlyParams = baseSchema.extend({
  timeEnd: z.coerce.string().min(1)
}).omit({ 
  userId: true
});
...
export type NewStatsUserMonthlyParams = z.infer<typeof insertStatsUserMonthlyParams>;
export type UpdateStatsUserMonthlyParams = z.infer<typeof updateStatsUserMonthlyParams>;
...
export type CompleteStatsUserMonthly = Awaited<ReturnType<typeof getStatsUserMonthlies>>["statsUserMonthlies"][number];
