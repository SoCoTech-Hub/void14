...
export const insertRoleAssignmentSchema = createInsertSchema(roleAssignments).omit(timestamps);
export const insertRoleAssignmentParams = baseSchema.extend({
...
export const updateRoleAssignmentParams = baseSchema.extend({
  roleId: z.coerce.string().min(1),
  sortOrder: z.coerce.number()
}).omit({ 
  userId: true
...
export type NewRoleAssignmentParams = z.infer<typeof insertRoleAssignmentParams>;
export type UpdateRoleAssignmentParams = z.infer<typeof updateRoleAssignmentParams>;
...
export type CompleteRoleAssignment = Awaited<ReturnType<typeof getRoleAssignments>>["roleAssignments"][number];
