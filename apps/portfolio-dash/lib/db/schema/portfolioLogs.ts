...
import { type getPortfolioLogs } from "@/lib/api/portfolioLogs/queries";

import { nanoid } from "@/lib/utils";


export const portfolioLogs = pgTable('portfolio_logs', {
  ...
  portfolioInstanceId: varchar("portfolio_instance_id", { length: 256 }).references(() => portfolioInstances.id).notNull(),
  returnUrl: varchar("return_url", { length: 256 }).notNull(),
  portfolioTempdataId: varchar("portfolio_tempdata_id", { length: 256 }).references(() => portfolioTempdatas.id).notNull(),
  time: integer("time"),
  userId: varchar("user_id", { length: 256 }).notNull()
}, (portfolioLogs) => {
  return {
    portfolioInstanceIdIndex: uniqueIndex('portfolio_instance_id_idx').on(portfolioLogs.portfolioInstanceId),
  }
});
...
export const insertPortfolioLogParams = baseSchema.extend({
...
export const updatePortfolioLogParams = baseSchema.extend({
  portfolioInstanceId: z.coerce.string().min(1),
  portfolioTempdataId: z.coerce.string().min(1),
  time: z.coerce.number()
}).omit({ 
...
// this type infers the return from getPortfolioLogs() - meaning it will include any joins
export type CompletePortfolioLog = Awaited<ReturnType<typeof getPortfolioLogs>>["portfolioLogs"][number];
