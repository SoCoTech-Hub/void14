...
export const insertApplicationResponseSchema = createInsertSchema(applicationResponses).omit(timestamps);
export const insertApplicationResponseParams = baseSchema.extend({
...
export const updateApplicationResponseParams = baseSchema.extend({
  jobApplicationId: z.coerce.string().min(1)
}).omit({ 
  userId: true
});
...
export type NewApplicationResponse = z.infer<typeof insertApplicationResponseSchema>;
export type NewApplicationResponseParams = z.infer<typeof insertApplicationResponseParams>;
export type UpdateApplicationResponseParams = z.infer<typeof updateApplicationResponseParams>;
export type ApplicationResponseId = z.infer<typeof applicationResponseIdSchema>["id"];
    
// this type infers the return from getApplicationResponses() - meaning it will include any joins
export type CompleteApplicationResponse = Awaited<ReturnType<typeof getApplicationResponses>>["applicationResponses"][number];
