...
import { varchar, integer, text, boolean, timestamp, pgTable } from "drizzle-orm/pg-core";
...
export const insertTaskLogSchema = createInsertSchema(taskLogs).omit(timestamps);
export const insertTaskLogParams = baseSchema.extend({
...
export const updateTaskLogParams = baseSchema.extend({
  dbReads: z.coerce.number(),
  dbWrites: z.coerce.number(),
  result: z.coerce.boolean(),
  type: z.coerce.number()
...
export type CompleteTaskLog = Awaited<ReturnType<typeof getTaskLogs>>["taskLogs"][number];
