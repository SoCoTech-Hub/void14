...
import { boolean, varchar, text, timestamp, pgTable } from "drizzle-orm/pg-core";
...
export const insertTaskAdhocSchema = createInsertSchema(taskAdhocs).omit(timestamps);
export const insertTaskAdhocParams = baseSchema.extend({
...
export const updateTaskAdhocParams = baseSchema.extend({
  blocking: z.coerce.boolean(),
  nextRuntime: z.coerce.string().min(1)
}).omit({ 
  userId: true
...
export type CompleteTaskAdhoc = Awaited<ReturnType<typeof getTaskAdhocs>>["taskAdhocs"][number];
