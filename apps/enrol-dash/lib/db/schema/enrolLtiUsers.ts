...
import { text, timestamp, real, varchar, pgTable } from "drizzle-orm/pg-core";
...
export const enrolLtiUsers = pgTable('enrol_lti_users', {
  ...
  enrolLtiDeploymentId: varchar("enrol_lti_deployment_id", { length: 256 }).references(() => enrolLtiDeployments.id, { onDelete: "cascade" }).notNull(),
...
export const insertEnrolLtiUserSchema = createInsertSchema(enrolLtiUsers).omit(timestamps);
export const insertEnrolLtiUserParams = baseSchema.extend({
...
export const updateEnrolLtiUserParams = baseSchema.extend({
  lastAccess: z.coerce.string().min(1),
  lastGrade: z.coerce.number(),
  enrolLtiDeploymentId: z.coerce.string().min(1)
}).omit({ 
...
export type CompleteEnrolLtiUser = Awaited<ReturnType<typeof getEnrolLtiUsers>>["enrolLtiUsers"][number];
