...
export const questionVersions = pgTable('question_versions', {
  ...
  questionBankEntryId: varchar("question_bank_entry_id", { length: 256 }).references(() => questionBankEntries.id).notNull(),
  questionId: varchar("question_id", { length: 256 }).references(() => questions.id, { onDelete: "cascade" }).notNull(),
  status: varchar("status", { length: 256 }),
  version: integer("version").notNull()
}, (questionVersions) => {
  return {
    questionIdIndex: uniqueIndex('question_id_idx').on(questionVersions.questionId),
  }
});
...
export const insertQuestionVersionParams = baseSchema.extend({
...
export type NewQuestionVersionParams = z.infer<typeof insertQuestionVersionParams>;
export type UpdateQuestionVersionParams = z.infer<typeof updateQuestionVersionParams>;
...
export type CompleteQuestionVersion = Awaited<ReturnType<typeof getQuestionVersions>>["questionVersions"][number];
