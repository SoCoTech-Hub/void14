...
export const questionCalculateds = pgTable('question_calculateds', {
  id: varchar("id", { length: 191 }).primaryKey().$defaultFn(() => nanoid()),
  questionAnswerId: varchar("question_answer_id", { length: 256 }).references(() => questionAnswers.id, { onDelete: "cascade" }).notNull(),
  ...
  return {
    questionIdIndex: uniqueIndex('question_id_idx').on(questionCalculateds.questionId),
  }
});
...
export const insertQuestionCalculatedSchema = createInsertSchema(questionCalculateds);
export const insertQuestionCalculatedParams = baseSchema.extend({
...
export type NewQuestionCalculated = z.infer<typeof insertQuestionCalculatedSchema>;
export type NewQuestionCalculatedParams = z.infer<typeof insertQuestionCalculatedParams>;
export type UpdateQuestionCalculatedParams = z.infer<typeof updateQuestionCalculatedParams>;
export type QuestionCalculatedId = z.infer<typeof questionCalculatedIdSchema>["id"];
    
// this type infers the return from getQuestionCalculateds() - meaning it will include any joins
export type CompleteQuestionCalculated = Awaited<ReturnType<typeof getQuestionCalculateds>>["questionCalculateds"][number];
