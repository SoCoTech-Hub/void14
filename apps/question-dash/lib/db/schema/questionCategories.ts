...
export const questionCategories = pgTable('question_categories', {
  ...
  return {
    contextIdIndex: uniqueIndex('context_id_idx').on(questionCategories.contextId),
  }
});
...
export const insertQuestionCategoryParams = baseSchema.extend({
...
export type NewQuestionCategoryParams = z.infer<typeof insertQuestionCategoryParams>;
export type UpdateQuestionCategoryParams = z.infer<typeof updateQuestionCategoryParams>;
...
export type CompleteQuestionCategory = Awaited<ReturnType<typeof getQuestionCategories>>["questionCategories"][number];
