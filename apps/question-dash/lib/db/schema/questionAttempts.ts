...
import { varchar, boolean, real, text, integer, timestamp, pgTable } from "drizzle-orm/pg-core";
...
export const questionAttempts = pgTable('question_attempts', {
  ...
  questionUsageId: varchar("question_usage_id", { length: 256 }).references(() => questionUsages.id).notNull(),
  ...
  return {
    questionIdIndex: uniqueIndex('question_id_idx').on(questionAttempts.questionId),
  }
});
...
export const insertQuestionAttemptSchema = createInsertSchema(questionAttempts).omit(timestamps);
export const insertQuestionAttemptParams = baseSchema.extend({
...
export type NewQuestionAttemptParams = z.infer<typeof insertQuestionAttemptParams>;
export type UpdateQuestionAttemptParams = z.infer<typeof updateQuestionAttemptParams>;
...
export type CompleteQuestionAttempt = Awaited<ReturnType<typeof getQuestionAttempts>>["questionAttempts"][number];
