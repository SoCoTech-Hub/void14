...
import { real, varchar, boolean, text, integer, timestamp, pgTable } from "drizzle-orm/pg-core";
...
export const questionStatistics = pgTable('question_statistics', {
  ...
  return {
    questionIdIndex: uniqueIndex('question_id_idx').on(questionStatistics.questionId),
  }
});
...
export const insertQuestionStatisticSchema = createInsertSchema(questionStatistics).omit(timestamps);
export const insertQuestionStatisticParams = baseSchema.extend({
...
export type NewQuestionStatisticParams = z.infer<typeof insertQuestionStatisticParams>;
export type UpdateQuestionStatisticParams = z.infer<typeof updateQuestionStatisticParams>;
export type QuestionStatisticId = z.infer<typeof questionStatisticIdSchema>["id"];
    
// this type infers the return from getQuestionStatistics() - meaning it will include any joins
export type CompleteQuestionStatistic = Awaited<ReturnType<typeof getQuestionStatistics>>["questionStatistics"][number];
