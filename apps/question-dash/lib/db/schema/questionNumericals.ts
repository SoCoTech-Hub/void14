...
export const questionNumericals = pgTable('question_numericals', {
  id: varchar("id", { length: 191 }).primaryKey().$defaultFn(() => nanoid()),
  questionAnswerId: varchar("question_answer_id", { length: 256 }).references(() => questionAnswers.id).notNull(),
  ...
  return {
    questionIdIndex: uniqueIndex('question_id_idx').on(questionNumericals.questionId),
  }
});
...
export const insertQuestionNumericalSchema = createInsertSchema(questionNumericals);
export const insertQuestionNumericalParams = baseSchema.extend({
...
export type NewQuestionNumericalParams = z.infer<typeof insertQuestionNumericalParams>;
export type UpdateQuestionNumericalParams = z.infer<typeof updateQuestionNumericalParams>;
export type QuestionNumericalId = z.infer<typeof questionNumericalIdSchema>["id"];
    
// this type infers the return from getQuestionNumericals() - meaning it will include any joins
export type CompleteQuestionNumerical = Awaited<ReturnType<typeof getQuestionNumericals>>["questionNumericals"][number];
