...
export const questionReferences = pgTable('question_references', {
  ...
  questionBankEntryId: varchar("question_bank_entry_id", { length: 256 }).references(() => questionBankEntries.id, { onDelete: "cascade" }).notNull(),
  usingContextId: varchar("using_context_id", { length: 256 }),
  version: integer("version")
}, (questionReferences) => {
  return {
    questionBankEntryIdIndex: uniqueIndex('question_bank_entry_id_idx').on(questionReferences.questionBankEntryId),
  }
});
...
export const insertQuestionReferenceSchema = createInsertSchema(questionReferences);
export const insertQuestionReferenceParams = baseSchema.extend({
...
export type NewQuestionReference = z.infer<typeof insertQuestionReferenceSchema>;
export type NewQuestionReferenceParams = z.infer<typeof insertQuestionReferenceParams>;
export type UpdateQuestionReferenceParams = z.infer<typeof updateQuestionReferenceParams>;
export type QuestionReferenceId = z.infer<typeof questionReferenceIdSchema>["id"];
    
// this type infers the return from getQuestionReferences() - meaning it will include any joins
export type CompleteQuestionReference = Awaited<ReturnType<typeof getQuestionReferences>>["questionReferences"][number];
