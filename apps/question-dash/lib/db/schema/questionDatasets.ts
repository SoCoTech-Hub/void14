...
export const questionDatasets = pgTable('question_datasets', {
  id: varchar("id", { length: 191 }).primaryKey().$defaultFn(() => nanoid()),
  questionDatasetDefinitionId: varchar("question_dataset_definition_id", { length: 256 }).references(() => questionDatasetDefinitions.id).notNull(),
  questionId: varchar("question_id", { length: 256 }).references(() => questions.id, { onDelete: "cascade" }).notNull()
}, (questionDatasets) => {
  return {
    questionIdIndex: uniqueIndex('question_id_idx').on(questionDatasets.questionId),
  }
});
...
export const insertQuestionDatasetParams = baseSchema.extend({
...
export type NewQuestionDatasetParams = z.infer<typeof insertQuestionDatasetParams>;
export type UpdateQuestionDatasetParams = z.infer<typeof updateQuestionDatasetParams>;
export type QuestionDatasetId = z.infer<typeof questionDatasetIdSchema>["id"];
    
// this type infers the return from getQuestionDatasets() - meaning it will include any joins
export type CompleteQuestionDataset = Awaited<ReturnType<typeof getQuestionDatasets>>["questionDatasets"][number];
