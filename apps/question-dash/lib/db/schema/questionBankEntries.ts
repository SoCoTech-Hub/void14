...
export const questionBankEntries = pgTable('question_bank_entries', {
  ...
  return {
    questionCategoryIdIndex: uniqueIndex('question_category_id_idx').on(questionBankEntries.questionCategoryId),
  }
});
...
export const insertQuestionBankEntrySchema = createInsertSchema(questionBankEntries);
export const insertQuestionBankEntryParams = baseSchema.extend({
...
export const updateQuestionBankEntryParams = baseSchema.extend({
  questionCategoryId: z.coerce.string().min(1)
}).omit({ 
  userId: true
});
...
export type NewQuestionBankEntryParams = z.infer<typeof insertQuestionBankEntryParams>;
export type UpdateQuestionBankEntryParams = z.infer<typeof updateQuestionBankEntryParams>;
export type QuestionBankEntryId = z.infer<typeof questionBankEntryIdSchema>["id"];
    
// this type infers the return from getQuestionBankEntries() - meaning it will include any joins
export type CompleteQuestionBankEntry = Awaited<ReturnType<typeof getQuestionBankEntries>>["questionBankEntries"][number];
