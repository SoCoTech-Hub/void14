...
export async function POST(req: Request) {
  try {
    const validatedData = insertQuestionNumericalUnitParams.parse(await req.json());
    const { questionNumericalUnit } = await createQuestionNumericalUnit(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateQuestionNumericalUnitParams.parse(await req.json());
    const validatedParams = questionNumericalUnitIdSchema.parse({ id });

    const { questionNumericalUnit } = await updateQuestionNumericalUnit(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = questionNumericalUnitIdSchema.parse({ id });
    const { questionNumericalUnit } = await deleteQuestionNumericalUnit(validatedParams.id);
