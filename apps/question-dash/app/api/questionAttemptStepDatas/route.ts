...
export async function POST(req: Request) {
  try {
    const validatedData = insertQuestionAttemptStepDataParams.parse(await req.json());
    const { questionAttemptStepData } = await createQuestionAttemptStepData(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateQuestionAttemptStepDataParams.parse(await req.json());
    const validatedParams = questionAttemptStepDataIdSchema.parse({ id });

    const { questionAttemptStepData } = await updateQuestionAttemptStepData(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = questionAttemptStepDataIdSchema.parse({ id });
    const { questionAttemptStepData } = await deleteQuestionAttemptStepData(validatedParams.id);
