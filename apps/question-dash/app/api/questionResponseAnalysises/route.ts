...
export async function POST(req: Request) {
  try {
    const validatedData = insertQuestionResponseAnalysiseParams.parse(await req.json());
    const { questionResponseAnalysise } = await createQuestionResponseAnalysise(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateQuestionResponseAnalysiseParams.parse(await req.json());
    const validatedParams = questionResponseAnalysiseIdSchema.parse({ id });

    const { questionResponseAnalysise } = await updateQuestionResponseAnalysise(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = questionResponseAnalysiseIdSchema.parse({ id });
    const { questionResponseAnalysise } = await deleteQuestionResponseAnalysise(validatedParams.id);
