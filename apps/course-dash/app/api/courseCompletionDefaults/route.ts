...
export async function POST(req: Request) {
  try {
    const validatedData = insertCourseCompletionDefaultParams.parse(await req.json());
    const { courseCompletionDefault } = await createCourseCompletionDefault(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateCourseCompletionDefaultParams.parse(await req.json());
    const validatedParams = courseCompletionDefaultIdSchema.parse({ id });

    const { courseCompletionDefault } = await updateCourseCompletionDefault(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = courseCompletionDefaultIdSchema.parse({ id });
    const { courseCompletionDefault } = await deleteCourseCompletionDefault(validatedParams.id);
