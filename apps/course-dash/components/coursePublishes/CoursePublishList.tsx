...
export default function CoursePublishList({
	...
	coursePublishes: CompleteCoursePublish[]
}) {
	const { data: c } = trpc.coursePublishes.getCoursePublishes.useQuery(
		undefined,
		{
...
const CoursePublish = ({
	...
	coursePublish: CompleteCoursePublish
}) => {
	return (
		<li className='flex justify-between my-2'>
			<div className='w-full'>
				<div>{coursePublish.courseId}</div>
			</div>
			<CoursePublishModal coursePublish={coursePublish} />
		</li>
	)
}
