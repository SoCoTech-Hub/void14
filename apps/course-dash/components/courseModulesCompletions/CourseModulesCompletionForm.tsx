...
import { CourseModulesCompletion, NewCourseModulesCompletionParams, insertCourseModulesCompletionParams } from "@/lib/db/schema/courseModulesCompletions";
...
const CourseModulesCompletionForm = ({
  ...
  const handleSubmit = (values: NewCourseModulesCompletionParams) => {
    if (editing) {
      updateCourseModulesCompletion({ ...values, id: courseModulesCompletion.id });
    } else {
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className={"space-y-8"}>
        ...
        {editing ? (
          <Button
            type="button"
            variant={"destructive"}
            onClick={() => deleteCourseModulesCompletion({ id: courseModulesCompletion.id })}
          >
