...
import { type getCourseCompletions } from "@/lib/api/courseCompletions/queries";
...
export const courseCompletions = pgTable('course_completions', {
  ...
  return {
    courseIdIndex: uniqueIndex('course_id_idx').on(courseCompletions.courseId),
  }
});
...
export const insertCourseCompletionSchema = createInsertSchema(courseCompletions);
export const insertCourseCompletionParams = baseSchema.extend({
...
export const updateCourseCompletionParams = baseSchema.extend({
  courseId: z.coerce.string().min(1),
  timeCompleted: z.coerce.string().min(1),
  timeEnrolled: z.coerce.string().min(1),
  timeStarted: z.coerce.string().min(1)
...
export type NewCourseCompletionParams = z.infer<typeof insertCourseCompletionParams>;
export type UpdateCourseCompletionParams = z.infer<typeof updateCourseCompletionParams>;
...
export type CompleteCourseCompletion = Awaited<ReturnType<typeof getCourseCompletions>>["courseCompletions"][number];
