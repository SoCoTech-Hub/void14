...
export const coursePublishes = pgTable('course_publishes', {
  ...
  return {
    courseIdIndex: uniqueIndex('course_id_idx').on(coursePublishes.courseId),
  }
});
...
export const insertCoursePublishSchema = createInsertSchema(coursePublishes).omit(timestamps);
export const insertCoursePublishParams = baseSchema.extend({
...
export type UpdateCoursePublishParams = z.infer<typeof updateCoursePublishParams>;
...
export type CompleteCoursePublish = Awaited<ReturnType<typeof getCoursePublishes>>["coursePublishes"][number];
