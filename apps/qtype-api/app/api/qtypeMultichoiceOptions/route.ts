...
export async function POST(req: Request) {
	try {
		const validatedData = insertQtypeMultichoiceOptionParams.parse(
			await req.json()
		)
		const { qtypeMultichoiceOption } =
			await createQtypeMultichoiceOption(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateQtypeMultichoiceOptionParams.parse(
			await req.json()
		)
		const validatedParams = qtypeMultichoiceOptionIdSchema.parse({ id })

		const { qtypeMultichoiceOption } = await updateQtypeMultichoiceOption(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = qtypeMultichoiceOptionIdSchema.parse({ id })
		const { qtypeMultichoiceOption } = await deleteQtypeMultichoiceOption(
			validatedParams.id
		)
