...
export async function POST(req: Request) {
	try {
		const validatedData = insertQtypeShortanswerOptionParams.parse(
			await req.json()
		)
		const { qtypeShortanswerOption } =
			await createQtypeShortanswerOption(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateQtypeShortanswerOptionParams.parse(
			await req.json()
		)
		const validatedParams = qtypeShortanswerOptionIdSchema.parse({ id })

		const { qtypeShortanswerOption } = await updateQtypeShortanswerOption(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = qtypeShortanswerOptionIdSchema.parse({ id })
		const { qtypeShortanswerOption } = await deleteQtypeShortanswerOption(
			validatedParams.id
		)
