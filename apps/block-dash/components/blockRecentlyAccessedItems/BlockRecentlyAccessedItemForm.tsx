...
import { BlockRecentlyAccessedItem, NewBlockRecentlyAccessedItemParams, insertBlockRecentlyAccessedItemParams } from "@/lib/db/schema/blockRecentlyAccessedItems";
...
const BlockRecentlyAccessedItemForm = ({
  ...
  const handleSubmit = (values: NewBlockRecentlyAccessedItemParams) => {
    if (editing) {
      updateBlockRecentlyAccessedItem({ ...values, id: blockRecentlyAccessedItem.id });
    } else {
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className={"space-y-8"}>
        ...
        {editing ? (
          <Button
            type="button"
            variant={"destructive"}
            onClick={() => deleteBlockRecentlyAccessedItem({ id: blockRecentlyAccessedItem.id })}
          >
