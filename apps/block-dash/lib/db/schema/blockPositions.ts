...
export const blockPositions = pgTable('block_positions', {
  ...
  blockInstanceId: varchar("block_instance_id", { length: 256 }).references(() => blockInstances.id).notNull(),
  ...
  return {
    blockInstanceIdIndex: uniqueIndex('block_instance_id_idx').on(blockPositions.blockInstanceId),
  }
});
...
export const insertBlockPositionParams = baseSchema.extend({
...
export type UpdateBlockPositionParams = z.infer<typeof updateBlockPositionParams>;
...
export type CompleteBlockPosition = Awaited<ReturnType<typeof getBlockPositions>>["blockPositions"][number];
