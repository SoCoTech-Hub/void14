...
export async function POST(req: Request) {
	try {
		const validatedData = insertH5pLibraryDependencyParams.parse(
			await req.json()
		)
		const { h5pLibraryDependency } =
			await createH5pLibraryDependency(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateH5pLibraryDependencyParams.parse(
			await req.json()
		)
		const validatedParams = h5pLibraryDependencyIdSchema.parse({ id })

		const { h5pLibraryDependency } = await updateH5pLibraryDependency(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = h5pLibraryDependencyIdSchema.parse({ id })
		const { h5pLibraryDependency } = await deleteH5pLibraryDependency(
			validatedParams.id
		)
