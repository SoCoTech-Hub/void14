...
export const oauth2Endpoints = pgTable('oauth2_endpoints', {
  ...
  return {
    oauth2issuerIdIndex: uniqueIndex('oauth2issuer_id_idx').on(oauth2Endpoints.oauth2issuerId),
  }
});
...
export const insertOauth2EndpointSchema = createInsertSchema(oauth2Endpoints).omit(timestamps);
export const insertOauth2EndpointParams = baseSchema.extend({
...
export const updateOauth2EndpointParams = baseSchema.extend({
  oauth2issuerId: z.coerce.string().min(1)
}).omit({ 
  userId: true
});
...
export type NewOauth2EndpointParams = z.infer<typeof insertOauth2EndpointParams>;
export type UpdateOauth2EndpointParams = z.infer<typeof updateOauth2EndpointParams>;
...
export type CompleteOauth2Endpoint = Awaited<ReturnType<typeof getOauth2Endpoints>>["oauth2Endpoints"][number];
