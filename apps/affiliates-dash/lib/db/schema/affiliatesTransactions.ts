...
export const affiliatesTransactions = pgTable('affiliates_transactions', {
  ...
  affiliateId: varchar("affiliate_id", { length: 256 }).references(() => affiliates.id, { onDelete: "cascade" }).notNull(),
  affiliatesStatusId: varchar("affiliates_status_id", { length: 256 }).references(() => affiliatesStatuses.id, { onDelete: "cascade" }).notNull()
});
...
export const insertAffiliatesTransactionSchema = createInsertSchema(affiliatesTransactions);
export const insertAffiliatesTransactionParams = baseSchema.extend({
...
export type NewAffiliatesTransaction = z.infer<typeof insertAffiliatesTransactionSchema>;
export type NewAffiliatesTransactionParams = z.infer<typeof insertAffiliatesTransactionParams>;
export type UpdateAffiliatesTransactionParams = z.infer<typeof updateAffiliatesTransactionParams>;
export type AffiliatesTransactionId = z.infer<typeof affiliatesTransactionIdSchema>["id"];
    
// this type infers the return from getAffiliatesTransactions() - meaning it will include any joins
export type CompleteAffiliatesTransaction = Awaited<ReturnType<typeof getAffiliatesTransactions>>["affiliatesTransactions"][number];
