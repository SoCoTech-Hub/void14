...
import { AffiliatesTransaction, NewAffiliatesTransactionParams, insertAffiliatesTransactionParams } from "@/lib/db/schema/affiliatesTransactions";
...
const AffiliatesTransactionForm = ({
  ...
  const { data: affiliatesStatuses } = trpc.affiliatesStatuses.getAffiliatesStatuses.useQuery();
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className={"space-y-8"}>
                  ...
                  <SelectContent>
                    {affiliates?.affiliates.map((affiliate) => (
                      <SelectItem key={affiliate.id} value={affiliate.id.toString()}>
                        {affiliate.note}  {/* TODO: Replace with a field from the affiliate model */}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
          ...
          render={({ field }) => (<FormItem>
              <FormLabel>Affiliates Status Id</FormLabel>
                ...
                >
                  ...
                  <SelectContent>
                    {affiliatesStatuses?.affiliatesStatuses.map((affiliatesStatus) => (
                      <SelectItem key={affiliatesStatus.id} value={affiliatesStatus.id.toString()}>
                        {affiliatesStatus.name}  {/* TODO: Replace with a field from the affiliatesStatus model */}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
        ...
        {editing ? (
          <Button
            type="button"
            variant={"destructive"}
            onClick={() => deleteAffiliatesTransaction({ id: affiliatesTransaction.id })}
          >
