...
export async function POST(req: Request) {
  try {
    const validatedData = insertWorkshopAllocationScheduleParams.parse(await req.json());
    const { workshopAllocationSchedule } = await createWorkshopAllocationSchedule(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateWorkshopAllocationScheduleParams.parse(await req.json());
    const validatedParams = workshopAllocationScheduleIdSchema.parse({ id });

    const { workshopAllocationSchedule } = await updateWorkshopAllocationSchedule(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = workshopAllocationScheduleIdSchema.parse({ id });
    const { workshopAllocationSchedule } = await deleteWorkshopAllocationSchedule(validatedParams.id);
