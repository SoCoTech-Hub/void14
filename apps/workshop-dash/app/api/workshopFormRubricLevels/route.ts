...
export async function POST(req: Request) {
  try {
    const validatedData = insertWorkshopFormRubricLevelParams.parse(await req.json());
    const { workshopFormRubricLevel } = await createWorkshopFormRubricLevel(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateWorkshopFormRubricLevelParams.parse(await req.json());
    const validatedParams = workshopFormRubricLevelIdSchema.parse({ id });

    const { workshopFormRubricLevel } = await updateWorkshopFormRubricLevel(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = workshopFormRubricLevelIdSchema.parse({ id });
    const { workshopFormRubricLevel } = await deleteWorkshopFormRubricLevel(validatedParams.id);
