...
export const workshopGrades = pgTable('workshop_grades', {
  ...
  return {
    assessmentIdIndex: uniqueIndex('assessment_id_idx').on(workshopGrades.assessmentId),
  }
});
...
export const insertWorkshopGradeParams = baseSchema.extend({
...
export const updateWorkshopGradeParams = baseSchema.extend({
  grade: z.coerce.number(),
  peerCommentFormat: z.coerce.number()
}).omit({ 
  userId: true
...
export type UpdateWorkshopGradeParams = z.infer<typeof updateWorkshopGradeParams>;
...
export type CompleteWorkshopGrade = Awaited<ReturnType<typeof getWorkshopGrades>>["workshopGrades"][number];
