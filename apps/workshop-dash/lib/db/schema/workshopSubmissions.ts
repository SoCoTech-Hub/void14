...
import { integer, text, varchar, real, boolean, timestamp, pgTable } from "drizzle-orm/pg-core";
...
export const workshopSubmissions = pgTable('workshop_submissions', {
  ...
  return {
    workshopIdIndex: uniqueIndex('workshop_id_idx').on(workshopSubmissions.workshopId),
  }
});
...
export const insertWorkshopSubmissionSchema = createInsertSchema(workshopSubmissions).omit(timestamps);
export const insertWorkshopSubmissionParams = baseSchema.extend({
...
export const updateWorkshopSubmissionParams = baseSchema.extend({
  attachment: z.coerce.number(),
  contentFormat: z.coerce.number(),
  contentTrust: z.coerce.number(),
  example: z.coerce.number(),
...
export type NewWorkshopSubmission = z.infer<typeof insertWorkshopSubmissionSchema>;
export type NewWorkshopSubmissionParams = z.infer<typeof insertWorkshopSubmissionParams>;
export type UpdateWorkshopSubmissionParams = z.infer<typeof updateWorkshopSubmissionParams>;
export type WorkshopSubmissionId = z.infer<typeof workshopSubmissionIdSchema>["id"];
    
// this type infers the return from getWorkshopSubmissions() - meaning it will include any joins
export type CompleteWorkshopSubmission = Awaited<ReturnType<typeof getWorkshopSubmissions>>["workshopSubmissions"][number];
