...
import { WorkshopAllocationSchedule, NewWorkshopAllocationScheduleParams, insertWorkshopAllocationScheduleParams } from "@/lib/db/schema/workshopAllocationSchedules";
...
const WorkshopAllocationScheduleForm = ({
  ...
  const handleSubmit = (values: NewWorkshopAllocationScheduleParams) => {
    if (editing) {
      updateWorkshopAllocationSchedule({ ...values, id: workshopAllocationSchedule.id });
    } else {
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className={"space-y-8"}>
        ...
        {editing ? (
          <Button
            type="button"
            variant={"destructive"}
            onClick={() => deleteWorkshopAllocationSchedule({ id: workshopAllocationSchedule.id })}
          >
