...
export const insertUserInfoDataParams = baseSchema.extend({
...
export const updateUserInfoDataParams = baseSchema.extend({
  dataFormat: z.coerce.number()
}).omit({ 
  userId: true
});
...
export type CompleteUserInfoData = Awaited<ReturnType<typeof getUserInfoDatas>>["userInfoDatas"][number];
