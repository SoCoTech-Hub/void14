...
export const insertUserEnrolmentSchema = createInsertSchema(userEnrolments).omit(timestamps);
export const insertUserEnrolmentParams = baseSchema.extend({
...
export const updateUserEnrolmentParams = baseSchema.extend({
  timeStart: z.coerce.string().min(1),
  timeEnd: z.coerce.string().min(1)
}).omit({ 
  userId: true
...
export type UpdateUserEnrolmentParams = z.infer<typeof updateUserEnrolmentParams>;
...
export type CompleteUserEnrolment = Awaited<ReturnType<typeof getUserEnrolments>>["userEnrolments"][number];
