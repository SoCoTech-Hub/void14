...
export async function POST(req: Request) {
  try {
    const validatedData = insertScormSeqRollupRuleCondParams.parse(await req.json());
    const { scormSeqRollupRuleCond } = await createScormSeqRollupRuleCond(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateScormSeqRollupRuleCondParams.parse(await req.json());
    const validatedParams = scormSeqRollupRuleCondIdSchema.parse({ id });

    const { scormSeqRollupRuleCond } = await updateScormSeqRollupRuleCond(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = scormSeqRollupRuleCondIdSchema.parse({ id });
    const { scormSeqRollupRuleCond } = await deleteScormSeqRollupRuleCond(validatedParams.id);
