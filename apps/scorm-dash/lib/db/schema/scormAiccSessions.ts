...
export const insertScormAiccSessionSchema = createInsertSchema(scormAiccSessions).omit(timestamps);
export const insertScormAiccSessionParams = baseSchema.extend({
...
export const updateScormAiccSessionParams = baseSchema.extend({
  attempt: z.coerce.number(),
  scormScoeId: z.coerce.string().min(1),
  scormId: z.coerce.string().min(1)
}).omit({ 
...
export type NewScormAiccSessionParams = z.infer<typeof insertScormAiccSessionParams>;
export type UpdateScormAiccSessionParams = z.infer<typeof updateScormAiccSessionParams>;
...
export type CompleteScormAiccSession = Awaited<ReturnType<typeof getScormAiccSessions>>["scormAiccSessions"][number];
