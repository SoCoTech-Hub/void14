...
export const insertSocialShareParams = baseSchema.extend({
...
export const updateSocialShareParams = baseSchema.extend({
  socialLinkId: z.coerce.string().min(1)
}).omit({ 
  userId: true
});
...
// this type infers the return from getSocialShares() - meaning it will include any joins
export type CompleteSocialShare = Awaited<ReturnType<typeof getSocialShares>>["socialShares"][number];
