...
export const insertSocialParams = baseSchema.extend({
...
export const updateSocialParams = baseSchema.extend({
  socialEmojiId: z.coerce.string().min(1)
}).omit({ 
  userId: true
});
...
export type CompleteSocial = Awaited<ReturnType<typeof getSocials>>["socials"][number];
