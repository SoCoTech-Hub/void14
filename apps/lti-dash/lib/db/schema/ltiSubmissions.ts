import { timestamp, real, varchar, integer, pgTable } from "drizzle-orm/pg-core";
...
export const insertLtiSubmissionParams = baseSchema.extend({
...
export const updateLtiSubmissionParams = baseSchema.extend({
  dateSubmitted: z.coerce.string().min(1),
  dateUpdated: z.coerce.string().min(1),
  gradePercent: z.coerce.number(),
  originalGrade: z.coerce.number(),
...
export type UpdateLtiSubmissionParams = z.infer<typeof updateLtiSubmissionParams>;
...
export type CompleteLtiSubmission = Awaited<ReturnType<typeof getLtiSubmissions>>["ltiSubmissions"][number];
