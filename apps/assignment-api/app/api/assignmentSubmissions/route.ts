...
export async function POST(req: Request) {
	try {
		const validatedData = insertAssignmentSubmissionParams.parse(
			await req.json()
		)
		const { assignmentSubmission } =
			await createAssignmentSubmission(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateAssignmentSubmissionParams.parse(
			await req.json()
		)
		const validatedParams = assignmentSubmissionIdSchema.parse({ id })

		const { assignmentSubmission } = await updateAssignmentSubmission(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = assignmentSubmissionIdSchema.parse({ id })
		const { assignmentSubmission } = await deleteAssignmentSubmission(
			validatedParams.id
		)
