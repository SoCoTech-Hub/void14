...
export async function POST(req: Request) {
	try {
		const validatedData = insertAssignSubmissionFileParams.parse(
			await req.json()
		)
		const { assignSubmissionFile } =
			await createAssignSubmissionFile(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateAssignSubmissionFileParams.parse(
			await req.json()
		)
		const validatedParams = assignSubmissionFileIdSchema.parse({ id })

		const { assignSubmissionFile } = await updateAssignSubmissionFile(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = assignSubmissionFileIdSchema.parse({ id })
		const { assignSubmissionFile } = await deleteAssignSubmissionFile(
			validatedParams.id
		)
