...
export const cohortMembers = pgTable('cohort_members', {
  ...
  cohortId: varchar("cohort_id", { length: 256 }).references(() => cohorts.id, { onDelete: "cascade" }).notNull(),
...
export const insertCohortMemberSchema = createInsertSchema(cohortMembers).omit(timestamps);
export const insertCohortMemberParams = baseSchema.extend({
...
export const updateCohortMemberParams = baseSchema.extend({
  cohortId: z.coerce.string().min(1)
}).omit({ 
  userId: true
});
...
// this type infers the return from getCohortMembers() - meaning it will include any joins
export type CompleteCohortMember = Awaited<ReturnType<typeof getCohortMembers>>["cohortMembers"][number];
