...
export const insertMessageReadSchema = createInsertSchema(messageReads).omit(timestamps);
export const insertMessageReadParams = baseSchema.extend({
...
export const updateMessageReadParams = baseSchema.extend({
  fullMessageFormat: z.coerce.number(),
  notification: z.coerce.boolean(),
  timeRead: z.coerce.string().min(1),
  timeUserFromDeleted: z.coerce.string().min(1),
...
export type CompleteMessageRead = Awaited<ReturnType<typeof getMessageReads>>["messageReads"][number];
