...
export async function PUT(req: Request) {
  try {
    ...
    const { roleAllowOverride } = await updateRoleAllowOverride(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = roleAllowOverrideIdSchema.parse({ id });
    const { roleAllowOverride } = await deleteRoleAllowOverride(validatedParams.id);
