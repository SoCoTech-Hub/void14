...
const MessageReadForm = ({
  ...
  const form = useForm<z.infer<typeof insertMessageReadParams>>({
  ...
  const { mutate: createMessageRead, isLoading: isCreating } =
    trpc.messageReads.createMessageRead.useMutation({
      onSuccess: (res) => onSuccess("create"),
      onError: (err) => onError("create", { error: err.message }),
    });

  const { mutate: updateMessageRead, isLoading: isUpdating } =
    trpc.messageReads.updateMessageRead.useMutation({
      onSuccess: (res) => onSuccess("update"),
      onError: (err) => onError("update", { error: err.message }),
    });

  const { mutate: deleteMessageRead, isLoading: isDeleting } =
    trpc.messageReads.deleteMessageRead.useMutation({
      onSuccess: (res) => onSuccess("delete"),
      onError: (err) => onError("delete", { error: err.message }),
    });

  const handleSubmit = (values: NewMessageReadParams) => {
    if (editing) {
      updateMessageRead({ ...values, id: messageRead.id });
    } else {
      createMessageRead(values);
    }
  };
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className={"space-y-8"}>
        ...
        {editing ? (
          <Button
            type="button"
            variant={"destructive"}
            onClick={() => deleteMessageRead({ id: messageRead.id })}
          >
...
export default MessageReadForm;
