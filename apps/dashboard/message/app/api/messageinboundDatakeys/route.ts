...
export async function POST(req: Request) {
  try {
    const validatedData = insertMessageinboundDatakeyParams.parse(await req.json());
    const { messageinboundDatakey } = await createMessageinboundDatakey(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateMessageinboundDatakeyParams.parse(await req.json());
    const validatedParams = messageinboundDatakeyIdSchema.parse({ id });

    const { messageinboundDatakey } = await updateMessageinboundDatakey(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = messageinboundDatakeyIdSchema.parse({ id });
    const { messageinboundDatakey } = await deleteMessageinboundDatakey(validatedParams.id);
