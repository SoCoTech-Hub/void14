...
export async function POST(req: Request) {
  try {
    const validatedData = insertMessagePopupNotificationParams.parse(await req.json());
    const { messagePopupNotification } = await createMessagePopupNotification(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateMessagePopupNotificationParams.parse(await req.json());
    const validatedParams = messagePopupNotificationIdSchema.parse({ id });

    const { messagePopupNotification } = await updateMessagePopupNotification(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = messagePopupNotificationIdSchema.parse({ id });
    const { messagePopupNotification } = await deleteMessagePopupNotification(validatedParams.id);
