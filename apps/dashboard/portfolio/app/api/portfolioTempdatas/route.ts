...
export async function PUT(req: Request) {
  try {
    ...
    const { portfolioTempdata } = await updatePortfolioTempdata(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = portfolioTempdataIdSchema.parse({ id });
    const { portfolioTempdata } = await deletePortfolioTempdata(validatedParams.id);
