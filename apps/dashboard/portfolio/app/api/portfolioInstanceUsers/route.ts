...
} from "@soco/portfolio-api/portfolioInstanceUsers/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertPortfolioInstanceUserParams.parse(await req.json());
    const { portfolioInstanceUser } = await createPortfolioInstanceUser(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updatePortfolioInstanceUserParams.parse(await req.json());
    const validatedParams = portfolioInstanceUserIdSchema.parse({ id });

    const { portfolioInstanceUser } = await updatePortfolioInstanceUser(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = portfolioInstanceUserIdSchema.parse({ id });
    const { portfolioInstanceUser } = await deletePortfolioInstanceUser(validatedParams.id);
