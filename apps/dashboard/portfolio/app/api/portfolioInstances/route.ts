...
} from "@soco/portfolio-api/portfolioInstances/mutations";
import { 
...
export async function PUT(req: Request) {
  try {
    ...
    const { portfolioInstance } = await updatePortfolioInstance(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = portfolioInstanceIdSchema.parse({ id });
    const { portfolioInstance } = await deletePortfolioInstance(validatedParams.id);
