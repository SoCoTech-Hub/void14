...
export async function POST(req: Request) {
  try {
    const validatedData = insertToolDataprivacyCategoryParams.parse(await req.json());
    const { toolDataprivacyCategory } = await createToolDataprivacyCategory(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateToolDataprivacyCategoryParams.parse(await req.json());
    const validatedParams = toolDataprivacyCategoryIdSchema.parse({ id });

    const { toolDataprivacyCategory } = await updateToolDataprivacyCategory(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = toolDataprivacyCategoryIdSchema.parse({ id });
    const { toolDataprivacyCategory } = await deleteToolDataprivacyCategory(validatedParams.id);
