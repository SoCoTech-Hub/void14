...
export async function POST(req: Request) {
  try {
    const validatedData = insertToolDataprivacyCtxLevelParams.parse(await req.json());
    const { toolDataprivacyCtxLevel } = await createToolDataprivacyCtxLevel(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateToolDataprivacyCtxLevelParams.parse(await req.json());
    const validatedParams = toolDataprivacyCtxLevelIdSchema.parse({ id });

    const { toolDataprivacyCtxLevel } = await updateToolDataprivacyCtxLevel(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = toolDataprivacyCtxLevelIdSchema.parse({ id });
    const { toolDataprivacyCtxLevel } = await deleteToolDataprivacyCtxLevel(validatedParams.id);
