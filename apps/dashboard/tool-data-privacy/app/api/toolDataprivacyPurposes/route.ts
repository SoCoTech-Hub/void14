...
export async function POST(req: Request) {
  try {
    const validatedData = insertToolDataprivacyPurposeParams.parse(await req.json());
    const { toolDataprivacyPurpose } = await createToolDataprivacyPurpose(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateToolDataprivacyPurposeParams.parse(await req.json());
    const validatedParams = toolDataprivacyPurposeIdSchema.parse({ id });

    const { toolDataprivacyPurpose } = await updateToolDataprivacyPurpose(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = toolDataprivacyPurposeIdSchema.parse({ id });
    const { toolDataprivacyPurpose } = await deleteToolDataprivacyPurpose(validatedParams.id);
