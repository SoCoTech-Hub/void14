...
} from "@soco/glossary-api/glossaryCategories/mutations";
import { 
...
export async function PUT(req: Request) {
  try {
    ...
    const { glossaryCategory } = await updateGlossaryCategory(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = glossaryCategoryIdSchema.parse({ id });
    const { glossaryCategory } = await deleteGlossaryCategory(validatedParams.id);
