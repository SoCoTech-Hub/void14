...
} from "@soco/admin-preset-api/adminpresetsAppItAs/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertAdminpresetsAppItAParams.parse(await req.json());
    const { adminpresetsAppItA } = await createAdminpresetsAppItA(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateAdminpresetsAppItAParams.parse(await req.json());
    const validatedParams = adminpresetsAppItAIdSchema.parse({ id });

    const { adminpresetsAppItA } = await updateAdminpresetsAppItA(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = adminpresetsAppItAIdSchema.parse({ id });
    const { adminpresetsAppItA } = await deleteAdminpresetsAppItA(validatedParams.id);
