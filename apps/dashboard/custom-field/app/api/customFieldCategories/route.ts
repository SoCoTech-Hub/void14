...
export async function POST(req: Request) {
  try {
    const validatedData = insertCustomFieldCategoryParams.parse(await req.json());
    const { customFieldCategory } = await createCustomFieldCategory(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateCustomFieldCategoryParams.parse(await req.json());
    const validatedParams = customFieldCategoryIdSchema.parse({ id });

    const { customFieldCategory } = await updateCustomFieldCategory(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = customFieldCategoryIdSchema.parse({ id });
    const { customFieldCategory } = await deleteCustomFieldCategory(validatedParams.id);
