...
} from "@soco/tool-policy-api/toolPolicyAcceptances/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertToolPolicyAcceptanceParams.parse(await req.json());
    const { toolPolicyAcceptance } = await createToolPolicyAcceptance(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateToolPolicyAcceptanceParams.parse(await req.json());
    const validatedParams = toolPolicyAcceptanceIdSchema.parse({ id });

    const { toolPolicyAcceptance } = await updateToolPolicyAcceptance(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = toolPolicyAcceptanceIdSchema.parse({ id });
    const { toolPolicyAcceptance } = await deleteToolPolicyAcceptance(validatedParams.id);
