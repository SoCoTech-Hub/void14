...
import { Show, NewShowParams, insertShowParams } from '@soco/show-db/schema/shows'
...
const ShowForm = ({
	...
	const { data: showsCategories } =
		trpc.showsCategories.getShowsCategories.useQuery()
								...
								>
									...
									<SelectContent>
										{showsCategories?.showsCategories.map((showsCategory) => (
											<SelectItem
												key={showsCategory.id}
												value={showsCategory.id.toString()}
											>
												{showsCategory.name}{' '}
												{/* TODO: Replace with a field from the showsCategory model */}
											</SelectItem>
										))}
									</SelectContent>
								</Select>
