...
const ExternalServicesUserForm = ({
  ...
  const form = useForm<z.infer<typeof insertExternalServicesUserParams>>({
  ...
  const { mutate: createExternalServicesUser, isLoading: isCreating } =
    trpc.externalServicesUsers.createExternalServicesUser.useMutation({
      onSuccess: (res) => onSuccess("create"),
      onError: (err) => onError("create", { error: err.message }),
    });

  const { mutate: updateExternalServicesUser, isLoading: isUpdating } =
    trpc.externalServicesUsers.updateExternalServicesUser.useMutation({
      onSuccess: (res) => onSuccess("update"),
      onError: (err) => onError("update", { error: err.message }),
    });

  const { mutate: deleteExternalServicesUser, isLoading: isDeleting } =
    trpc.externalServicesUsers.deleteExternalServicesUser.useMutation({
      onSuccess: (res) => onSuccess("delete"),
      onError: (err) => onError("delete", { error: err.message }),
    });

  const handleSubmit = (values: NewExternalServicesUserParams) => {
    if (editing) {
      updateExternalServicesUser({ ...values, id: externalServicesUser.id });
    } else {
      createExternalServicesUser(values);
    }
  };
  return (
    <Form {...form}>
...
export default ExternalServicesUserForm;
