...
} from "@soco/forum-api/forumDiscussionSubs/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertForumDiscussionSubParams.parse(await req.json());
    const { forumDiscussionSub } = await createForumDiscussionSub(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateForumDiscussionSubParams.parse(await req.json());
    const validatedParams = forumDiscussionSubIdSchema.parse({ id });

    const { forumDiscussionSub } = await updateForumDiscussionSub(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = forumDiscussionSubIdSchema.parse({ id });
    const { forumDiscussionSub } = await deleteForumDiscussionSub(validatedParams.id);
