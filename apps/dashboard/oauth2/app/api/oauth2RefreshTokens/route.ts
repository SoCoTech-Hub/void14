...
export async function PUT(req: Request) {
  try {
    ...
    const { oauth2RefreshToken } = await updateOauth2RefreshToken(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = oauth2RefreshTokenIdSchema.parse({ id });
    const { oauth2RefreshToken } = await deleteOauth2RefreshToken(validatedParams.id);
