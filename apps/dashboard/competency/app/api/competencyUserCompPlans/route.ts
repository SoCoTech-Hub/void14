...
} from "@soco/competency-api/competencyUserCompPlans/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertCompetencyUserCompPlanParams.parse(await req.json());
    const { competencyUserCompPlan } = await createCompetencyUserCompPlan(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateCompetencyUserCompPlanParams.parse(await req.json());
    const validatedParams = competencyUserCompPlanIdSchema.parse({ id });

    const { competencyUserCompPlan } = await updateCompetencyUserCompPlan(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = competencyUserCompPlanIdSchema.parse({ id });
    const { competencyUserCompPlan } = await deleteCompetencyUserCompPlan(validatedParams.id);
