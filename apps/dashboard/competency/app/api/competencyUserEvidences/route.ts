...
export async function POST(req: Request) {
  try {
    const validatedData = insertCompetencyUserEvidenceParams.parse(await req.json());
    const { competencyUserEvidence } = await createCompetencyUserEvidence(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateCompetencyUserEvidenceParams.parse(await req.json());
    const validatedParams = competencyUserEvidenceIdSchema.parse({ id });

    const { competencyUserEvidence } = await updateCompetencyUserEvidence(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = competencyUserEvidenceIdSchema.parse({ id });
    const { competencyUserEvidence } = await deleteCompetencyUserEvidence(validatedParams.id);
