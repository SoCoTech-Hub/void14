...
} from "@soco/competency-api/competencyRelatedComps/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertCompetencyRelatedCompParams.parse(await req.json());
    const { competencyRelatedComp } = await createCompetencyRelatedComp(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateCompetencyRelatedCompParams.parse(await req.json());
    const validatedParams = competencyRelatedCompIdSchema.parse({ id });

    const { competencyRelatedComp } = await updateCompetencyRelatedComp(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = competencyRelatedCompIdSchema.parse({ id });
    const { competencyRelatedComp } = await deleteCompetencyRelatedComp(validatedParams.id);
