...
} from "@soco/competency-api/competencyUserComps/mutations";
import { 
...
export async function PUT(req: Request) {
  try {
    ...
    const { competencyUserComp } = await updateCompetencyUserComp(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = competencyUserCompIdSchema.parse({ id });
    const { competencyUserComp } = await deleteCompetencyUserComp(validatedParams.id);
