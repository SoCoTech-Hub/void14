...
} from "@soco/competency-api/competencyModuleComps/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertCompetencyModuleCompParams.parse(await req.json());
    const { competencyModuleComp } = await createCompetencyModuleComp(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateCompetencyModuleCompParams.parse(await req.json());
    const validatedParams = competencyModuleCompIdSchema.parse({ id });

    const { competencyModuleComp } = await updateCompetencyModuleComp(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = competencyModuleCompIdSchema.parse({ id });
    const { competencyModuleComp } = await deleteCompetencyModuleComp(validatedParams.id);
