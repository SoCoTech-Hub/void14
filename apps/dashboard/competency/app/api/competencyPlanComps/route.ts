...
export async function PUT(req: Request) {
  try {
    ...
    const { competencyPlanComp } = await updateCompetencyPlanComp(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = competencyPlanCompIdSchema.parse({ id });
    const { competencyPlanComp } = await deleteCompetencyPlanComp(validatedParams.id);
