...
} from "@soco/competency-api/competencyCourseCompSettings/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertCompetencyCourseCompSettingParams.parse(await req.json());
    const { competencyCourseCompSetting } = await createCompetencyCourseCompSetting(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateCompetencyCourseCompSettingParams.parse(await req.json());
    const validatedParams = competencyCourseCompSettingIdSchema.parse({ id });

    const { competencyCourseCompSetting } = await updateCompetencyCourseCompSetting(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = competencyCourseCompSettingIdSchema.parse({ id });
    const { competencyCourseCompSetting } = await deleteCompetencyCourseCompSetting(validatedParams.id);
