...
export async function POST(req: Request) {
  try {
    const validatedData = insertGradingformRubricFillingParams.parse(await req.json());
    const { gradingformRubricFilling } = await createGradingformRubricFilling(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateGradingformRubricFillingParams.parse(await req.json());
    const validatedParams = gradingformRubricFillingIdSchema.parse({ id });

    const { gradingformRubricFilling } = await updateGradingformRubricFilling(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = gradingformRubricFillingIdSchema.parse({ id });
    const { gradingformRubricFilling } = await deleteGradingformRubricFilling(validatedParams.id);
