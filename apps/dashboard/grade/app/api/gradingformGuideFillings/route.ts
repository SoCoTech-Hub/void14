...
} from "@soco/grade-api/gradingformGuideFillings/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertGradingformGuideFillingParams.parse(await req.json());
    const { gradingformGuideFilling } = await createGradingformGuideFilling(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateGradingformGuideFillingParams.parse(await req.json());
    const validatedParams = gradingformGuideFillingIdSchema.parse({ id });

    const { gradingformGuideFilling } = await updateGradingformGuideFilling(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = gradingformGuideFillingIdSchema.parse({ id });
    const { gradingformGuideFilling } = await deleteGradingformGuideFilling(validatedParams.id);
