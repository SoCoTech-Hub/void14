...
export async function POST(req: Request) {
  try {
    const validatedData = insertGradingformGuideCommentParams.parse(await req.json());
    const { gradingformGuideComment } = await createGradingformGuideComment(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateGradingformGuideCommentParams.parse(await req.json());
    const validatedParams = gradingformGuideCommentIdSchema.parse({ id });

    const { gradingformGuideComment } = await updateGradingformGuideComment(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = gradingformGuideCommentIdSchema.parse({ id });
    const { gradingformGuideComment } = await deleteGradingformGuideComment(validatedParams.id);
