...
} from "@soco/grade-api/gradeOutcomesCourses/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertGradeOutcomesCourseParams.parse(await req.json());
    const { gradeOutcomesCourse } = await createGradeOutcomesCourse(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateGradeOutcomesCourseParams.parse(await req.json());
    const validatedParams = gradeOutcomesCourseIdSchema.parse({ id });

    const { gradeOutcomesCourse } = await updateGradeOutcomesCourse(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = gradeOutcomesCourseIdSchema.parse({ id });
    const { gradeOutcomesCourse } = await deleteGradeOutcomesCourse(validatedParams.id);
