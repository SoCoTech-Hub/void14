...
} from "@soco/grade-api/gradeImportNewitems/mutations";
import { 
...
export async function PUT(req: Request) {
  try {
    ...
    const { gradeImportNewitem } = await updateGradeImportNewitem(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = gradeImportNewitemIdSchema.parse({ id });
    const { gradeImportNewitem } = await deleteGradeImportNewitem(validatedParams.id);
