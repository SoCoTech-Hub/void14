...
} from "@soco/grade-api/gradeOutcomesHistories/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertGradeOutcomesHistoryParams.parse(await req.json());
    const { gradeOutcomesHistory } = await createGradeOutcomesHistory(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateGradeOutcomesHistoryParams.parse(await req.json());
    const validatedParams = gradeOutcomesHistoryIdSchema.parse({ id });

    const { gradeOutcomesHistory } = await updateGradeOutcomesHistory(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = gradeOutcomesHistoryIdSchema.parse({ id });
    const { gradeOutcomesHistory } = await deleteGradeOutcomesHistory(validatedParams.id);
