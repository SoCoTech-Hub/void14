...
} from "@soco/mnet-api/mnetHost2services/mutations";
import { 
...
export async function PUT(req: Request) {
  try {
    ...
    const { mnetHost2service } = await updateMnetHost2service(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = mnetHost2serviceIdSchema.parse({ id });
    const { mnetHost2service } = await deleteMnetHost2service(validatedParams.id);
