...
export async function POST(req: Request) {
  try {
    const validatedData = insertMnetServiceEnrolEnrolmentParams.parse(await req.json());
    const { mnetServiceEnrolEnrolment } = await createMnetServiceEnrolEnrolment(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateMnetServiceEnrolEnrolmentParams.parse(await req.json());
    const validatedParams = mnetServiceEnrolEnrolmentIdSchema.parse({ id });

    const { mnetServiceEnrolEnrolment } = await updateMnetServiceEnrolEnrolment(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = mnetServiceEnrolEnrolmentIdSchema.parse({ id });
    const { mnetServiceEnrolEnrolment } = await deleteMnetServiceEnrolEnrolment(validatedParams.id);
