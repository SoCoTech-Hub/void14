...
export async function PUT(req: Request) {
  try {
    ...
    const { analyticsUsedFile } = await updateAnalyticsUsedFile(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = analyticsUsedFileIdSchema.parse({ id });
    const { analyticsUsedFile } = await deleteAnalyticsUsedFile(validatedParams.id);
