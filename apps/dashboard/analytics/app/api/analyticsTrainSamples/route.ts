...
export async function POST(req: Request) {
  try {
    const validatedData = insertAnalyticsTrainSampleParams.parse(await req.json());
    const { analyticsTrainSample } = await createAnalyticsTrainSample(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateAnalyticsTrainSampleParams.parse(await req.json());
    const validatedParams = analyticsTrainSampleIdSchema.parse({ id });

    const { analyticsTrainSample } = await updateAnalyticsTrainSample(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = analyticsTrainSampleIdSchema.parse({ id });
    const { analyticsTrainSample } = await deleteAnalyticsTrainSample(validatedParams.id);
