...
export async function POST(req: Request) {
  try {
    const validatedData = insertReportbuilderReportParams.parse(await req.json());
    const { reportbuilderReport } = await createReportbuilderReport(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateReportbuilderReportParams.parse(await req.json());
    const validatedParams = reportbuilderReportIdSchema.parse({ id });

    const { reportbuilderReport } = await updateReportbuilderReport(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = reportbuilderReportIdSchema.parse({ id });
    const { reportbuilderReport } = await deleteReportbuilderReport(validatedParams.id);
