...
export async function POST(req: Request) {
  try {
    const validatedData = insertReportbuilderAudienceParams.parse(await req.json());
    const { reportbuilderAudience } = await createReportbuilderAudience(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateReportbuilderAudienceParams.parse(await req.json());
    const validatedParams = reportbuilderAudienceIdSchema.parse({ id });

    const { reportbuilderAudience } = await updateReportbuilderAudience(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = reportbuilderAudienceIdSchema.parse({ id });
    const { reportbuilderAudience } = await deleteReportbuilderAudience(validatedParams.id);
