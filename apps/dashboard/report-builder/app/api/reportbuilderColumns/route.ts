...
export async function POST(req: Request) {
  try {
    const validatedData = insertReportbuilderColumnParams.parse(await req.json());
    const { reportbuilderColumn } = await createReportbuilderColumn(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateReportbuilderColumnParams.parse(await req.json());
    const validatedParams = reportbuilderColumnIdSchema.parse({ id });

    const { reportbuilderColumn } = await updateReportbuilderColumn(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = reportbuilderColumnIdSchema.parse({ id });
    const { reportbuilderColumn } = await deleteReportbuilderColumn(validatedParams.id);
