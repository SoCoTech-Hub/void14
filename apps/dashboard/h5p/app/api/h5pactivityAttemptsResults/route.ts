...
} from "@soco/h5p-api/h5pactivityAttemptsResults/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertH5pactivityAttemptsResultParams.parse(await req.json());
    const { h5pactivityAttemptsResult } = await createH5pactivityAttemptsResult(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateH5pactivityAttemptsResultParams.parse(await req.json());
    const validatedParams = h5pactivityAttemptsResultIdSchema.parse({ id });

    const { h5pactivityAttemptsResult } = await updateH5pactivityAttemptsResult(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = h5pactivityAttemptsResultIdSchema.parse({ id });
    const { h5pactivityAttemptsResult } = await deleteH5pactivityAttemptsResult(validatedParams.id);
