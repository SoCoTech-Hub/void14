...
export async function POST(req: Request) {
  try {
    const validatedData = insertEnrolLtiLti2ToolProxyParams.parse(await req.json());
    const { enrolLtiLti2ToolProxy } = await createEnrolLtiLti2ToolProxy(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateEnrolLtiLti2ToolProxyParams.parse(await req.json());
    const validatedParams = enrolLtiLti2ToolProxyIdSchema.parse({ id });

    const { enrolLtiLti2ToolProxy } = await updateEnrolLtiLti2ToolProxy(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = enrolLtiLti2ToolProxyIdSchema.parse({ id });
    const { enrolLtiLti2ToolProxy } = await deleteEnrolLtiLti2ToolProxy(validatedParams.id);
