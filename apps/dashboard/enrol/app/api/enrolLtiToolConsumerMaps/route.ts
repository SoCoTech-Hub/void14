...
export async function POST(req: Request) {
  try {
    const validatedData = insertEnrolLtiToolConsumerMapParams.parse(await req.json());
    const { enrolLtiToolConsumerMap } = await createEnrolLtiToolConsumerMap(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateEnrolLtiToolConsumerMapParams.parse(await req.json());
    const validatedParams = enrolLtiToolConsumerMapIdSchema.parse({ id });

    const { enrolLtiToolConsumerMap } = await updateEnrolLtiToolConsumerMap(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = enrolLtiToolConsumerMapIdSchema.parse({ id });
    const { enrolLtiToolConsumerMap } = await deleteEnrolLtiToolConsumerMap(validatedParams.id);
