...
} from "@soco/enrol-api/enrolLtiLti2UserResults/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertEnrolLtiLti2UserResultParams.parse(await req.json());
    const { enrolLtiLti2UserResult } = await createEnrolLtiLti2UserResult(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateEnrolLtiLti2UserResultParams.parse(await req.json());
    const validatedParams = enrolLtiLti2UserResultIdSchema.parse({ id });

    const { enrolLtiLti2UserResult } = await updateEnrolLtiLti2UserResult(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = enrolLtiLti2UserResultIdSchema.parse({ id });
    const { enrolLtiLti2UserResult } = await deleteEnrolLtiLti2UserResult(validatedParams.id);
