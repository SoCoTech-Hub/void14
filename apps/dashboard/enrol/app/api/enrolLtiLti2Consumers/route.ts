...
export async function POST(req: Request) {
  try {
    const validatedData = insertEnrolLtiLti2ConsumerParams.parse(await req.json());
    const { enrolLtiLti2Consumer } = await createEnrolLtiLti2Consumer(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateEnrolLtiLti2ConsumerParams.parse(await req.json());
    const validatedParams = enrolLtiLti2ConsumerIdSchema.parse({ id });

    const { enrolLtiLti2Consumer } = await updateEnrolLtiLti2Consumer(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = enrolLtiLti2ConsumerIdSchema.parse({ id });
    const { enrolLtiLti2Consumer } = await deleteEnrolLtiLti2Consumer(validatedParams.id);
