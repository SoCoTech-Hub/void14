...
import { 
  type EnrolLtiToolId, 
  type NewEnrolLtiToolParams,
  type UpdateEnrolLtiToolParams, 
...
export const createEnrolLtiTool = async (enrolLtiTool: NewEnrolLtiToolParams) => {
  const newEnrolLtiTool = insertEnrolLtiToolSchema.parse(enrolLtiTool);
  try {
    const [e] =  await db.insert(enrolLtiTools).values(newEnrolLtiTool).returning();
...
export const updateEnrolLtiTool = async (id: EnrolLtiToolId, enrolLtiTool: UpdateEnrolLtiToolParams) => {
...
export const deleteEnrolLtiTool = async (id: EnrolLtiToolId) => {
  const { id: enrolLtiToolId } = enrolLtiToolIdSchema.parse({ id });
  try {
    const [e] =  await db.delete(enrolLtiTools).where(eq(enrolLtiTools.id, enrolLtiToolId!))
    .returning();
    return { enrolLtiTool: e };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
