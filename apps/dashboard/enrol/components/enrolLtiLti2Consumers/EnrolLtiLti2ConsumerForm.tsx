...
const EnrolLtiLti2ConsumerForm = ({
  ...
  const form = useForm<z.infer<typeof insertEnrolLtiLti2ConsumerParams>>({
  ...
  const { mutate: createEnrolLtiLti2Consumer, isLoading: isCreating } =
    trpc.enrolLtiLti2Consumers.createEnrolLtiLti2Consumer.useMutation({
      onSuccess: (res) => onSuccess("create"),
      onError: (err) => onError("create", { error: err.message }),
    });

  const { mutate: updateEnrolLtiLti2Consumer, isLoading: isUpdating } =
    trpc.enrolLtiLti2Consumers.updateEnrolLtiLti2Consumer.useMutation({
      onSuccess: (res) => onSuccess("update"),
      onError: (err) => onError("update", { error: err.message }),
    });

  const { mutate: deleteEnrolLtiLti2Consumer, isLoading: isDeleting } =
    trpc.enrolLtiLti2Consumers.deleteEnrolLtiLti2Consumer.useMutation({
      onSuccess: (res) => onSuccess("delete"),
      onError: (err) => onError("delete", { error: err.message }),
    });

  const handleSubmit = (values: NewEnrolLtiLti2ConsumerParams) => {
    if (editing) {
      updateEnrolLtiLti2Consumer({ ...values, id: enrolLtiLti2Consumer.id });
    } else {
      createEnrolLtiLti2Consumer(values);
    }
  };
  return (
    <Form {...form}>
...
export default EnrolLtiLti2ConsumerForm;
