...
export async function POST(req: Request) {
  try {
    const validatedData = insertBlockRecentActivityParams.parse(await req.json());
    const { blockRecentActivity } = await createBlockRecentActivity(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateBlockRecentActivityParams.parse(await req.json());
    const validatedParams = blockRecentActivityIdSchema.parse({ id });

    const { blockRecentActivity } = await updateBlockRecentActivity(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = blockRecentActivityIdSchema.parse({ id });
    const { blockRecentActivity } = await deleteBlockRecentActivity(validatedParams.id);
