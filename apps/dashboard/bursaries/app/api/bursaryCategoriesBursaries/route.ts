...
} from "@soco/bursaries-api/bursaryCategoriesBursaries/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertBursaryCategoriesBursaryParams.parse(await req.json());
    const { bursaryCategoriesBursary } = await createBursaryCategoriesBursary(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateBursaryCategoriesBursaryParams.parse(await req.json());
    const validatedParams = bursaryCategoriesBursaryIdSchema.parse({ id });

    const { bursaryCategoriesBursary } = await updateBursaryCategoriesBursary(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = bursaryCategoriesBursaryIdSchema.parse({ id });
    const { bursaryCategoriesBursary } = await deleteBursaryCategoriesBursary(validatedParams.id);
