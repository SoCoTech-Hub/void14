...
export async function PUT(req: Request) {
  try {
    ...
    const { qtypeDdmarkerDrag } = await updateQtypeDdmarkerDrag(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = qtypeDdmarkerDragIdSchema.parse({ id });
    const { qtypeDdmarkerDrag } = await deleteQtypeDdmarkerDrag(validatedParams.id);
