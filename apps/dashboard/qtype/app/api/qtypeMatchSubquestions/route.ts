...
export async function POST(req: Request) {
  try {
    const validatedData = insertQtypeMatchSubquestionParams.parse(await req.json());
    const { qtypeMatchSubquestion } = await createQtypeMatchSubquestion(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateQtypeMatchSubquestionParams.parse(await req.json());
    const validatedParams = qtypeMatchSubquestionIdSchema.parse({ id });

    const { qtypeMatchSubquestion } = await updateQtypeMatchSubquestion(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = qtypeMatchSubquestionIdSchema.parse({ id });
    const { qtypeMatchSubquestion } = await deleteQtypeMatchSubquestion(validatedParams.id);
