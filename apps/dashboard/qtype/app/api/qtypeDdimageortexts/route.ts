...
export async function PUT(req: Request) {
  try {
    ...
    const { qtypeDdimageortext } = await updateQtypeDdimageortext(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = qtypeDdimageortextIdSchema.parse({ id });
    const { qtypeDdimageortext } = await deleteQtypeDdimageortext(validatedParams.id);
