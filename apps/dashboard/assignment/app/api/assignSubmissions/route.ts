...
} from "@soco/assignment-api/assignSubmissions/mutations";
import { 
...
export async function PUT(req: Request) {
  try {
    ...
    const { assignSubmission } = await updateAssignSubmission(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = assignSubmissionIdSchema.parse({ id });
    const { assignSubmission } = await deleteAssignSubmission(validatedParams.id);
