...
} from "@soco/assignment-api/assignFeedbackEditpdfRots/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertAssignFeedbackEditpdfRotParams.parse(await req.json());
    const { assignFeedbackEditpdfRot } = await createAssignFeedbackEditpdfRot(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateAssignFeedbackEditpdfRotParams.parse(await req.json());
    const validatedParams = assignFeedbackEditpdfRotIdSchema.parse({ id });

    const { assignFeedbackEditpdfRot } = await updateAssignFeedbackEditpdfRot(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = assignFeedbackEditpdfRotIdSchema.parse({ id });
    const { assignFeedbackEditpdfRot } = await deleteAssignFeedbackEditpdfRot(validatedParams.id);
