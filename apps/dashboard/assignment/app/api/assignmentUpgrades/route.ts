...
export async function PUT(req: Request) {
  try {
    ...
    const { assignmentUpgrade } = await updateAssignmentUpgrade(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = assignmentUpgradeIdSchema.parse({ id });
    const { assignmentUpgrade } = await deleteAssignmentUpgrade(validatedParams.id);
