...
} from "@soco/subject-api/subjectsSubjectCategories/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertSubjectsSubjectCategoryParams.parse(await req.json());
    const { subjectsSubjectCategory } = await createSubjectsSubjectCategory(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateSubjectsSubjectCategoryParams.parse(await req.json());
    const validatedParams = subjectsSubjectCategoryIdSchema.parse({ id });

    const { subjectsSubjectCategory } = await updateSubjectsSubjectCategory(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = subjectsSubjectCategoryIdSchema.parse({ id });
    const { subjectsSubjectCategory } = await deleteSubjectsSubjectCategory(validatedParams.id);
