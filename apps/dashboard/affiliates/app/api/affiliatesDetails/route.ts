...
} from "@soco/affiliates-api/affiliatesDetails/mutations";
import { 
...
export async function PUT(req: Request) {
  try {
    ...
    const { affiliatesDetail } = await updateAffiliatesDetail(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = affiliatesDetailIdSchema.parse({ id });
    const { affiliatesDetail } = await deleteAffiliatesDetail(validatedParams.id);
