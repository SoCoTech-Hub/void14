...
import { Tag, NewTagParams, insertTagParams } from "@soco/tag-db/schema/tags";
...
const TagForm = ({
  ...
  const form = useForm<z.infer<typeof insertTagParams>>({
    // latest Zod release has introduced a TS error with zodResolver
    // open issue: https://github.com/colinhacks/zod/issues/2663
    // errors locally but not in production
    resolver: zodResolver(insertTagParams),
    defaultValues: tag ?? {
      description: "",
     descriptionFormat: 0,
     flag: 0,
  ...
  const onSuccess = async (action: "create" | "update" | "delete",
    ...
    await utils.tags.getTags.invalidate();
    router.refresh();
  ...
  const handleSubmit = (values: NewTagParams) => {
    if (editing) {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className={"space-y-8"}>
        ...
        <FormField
          ...
          name="isStandard"
          render={({ field }) => (<FormItem>
              <FormLabel>Is Standard</FormLabel>
                <br />
            <FormControl>
