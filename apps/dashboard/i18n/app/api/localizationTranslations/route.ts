...
} from "@soco/i18n-api/localizationTranslations/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertLocalizationTranslationParams.parse(await req.json());
    const { localizationTranslation } = await createLocalizationTranslation(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateLocalizationTranslationParams.parse(await req.json());
    const validatedParams = localizationTranslationIdSchema.parse({ id });

    const { localizationTranslation } = await updateLocalizationTranslation(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = localizationTranslationIdSchema.parse({ id });
    const { localizationTranslation } = await deleteLocalizationTranslation(validatedParams.id);
