...
export async function PUT(req: Request) {
  try {
    ...
    const { localizationUser } = await updateLocalizationUser(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = localizationUserIdSchema.parse({ id });
    const { localizationUser } = await deleteLocalizationUser(validatedParams.id);
