...
const LocalizationTranslationForm = ({
	...
	const { data: localizationFields } =
		trpc.localizationFields.getLocalizationFields.useQuery()
	const { data: localizationLanguages } =
		trpc.localizationLanguages.getLocalizationLanguages.useQuery()
			...
			>
						...
						<FormItem>
							...
							<FormControl>
								...
								>
									...
									<SelectContent>
										{localizationFields?.localizationFields.map(
											(localizationField) => (
												<SelectItem
													key={localizationField.id}
													value={localizationField.id.toString()}
												>
													{localizationField.name}{' '}
													{/* TODO: Replace with a field from the localizationField model */}
												</SelectItem>
											)
										)}
									</SelectContent>
						...
						<FormItem>
							...
							<FormControl>
								...
								>
									...
									<SelectContent>
										{localizationLanguages?.localizationLanguages.map(
											(localizationLanguage) => (
												<SelectItem
													key={localizationLanguage.id}
													value={localizationLanguage.id.toString()}
												>
													{localizationLanguage.name}{' '}
													{/* TODO: Replace with a field from the localizationLanguage model */}
												</SelectItem>
											)
										)}
									</SelectContent>
				...
				{editing ? (
					<Button
						...
						variant={'destructive'}
						onClick={() =>
							deleteLocalizationTranslation({ id: localizationTranslation.id })
						}
					>
