...
} from "@soco/repository-api/repositoryInstanceConfigs/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertRepositoryInstanceConfigParams.parse(await req.json());
    const { repositoryInstanceConfig } = await createRepositoryInstanceConfig(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateRepositoryInstanceConfigParams.parse(await req.json());
    const validatedParams = repositoryInstanceConfigIdSchema.parse({ id });

    const { repositoryInstanceConfig } = await updateRepositoryInstanceConfig(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = repositoryInstanceConfigIdSchema.parse({ id });
    const { repositoryInstanceConfig } = await deleteRepositoryInstanceConfig(validatedParams.id);
