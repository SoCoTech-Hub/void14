...
export async function PUT(req: Request) {
  try {
    ...
    const { userPasswordReset } = await updateUserPasswordReset(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = userPasswordResetIdSchema.parse({ id });
    const { userPasswordReset } = await deleteUserPasswordReset(validatedParams.id);
