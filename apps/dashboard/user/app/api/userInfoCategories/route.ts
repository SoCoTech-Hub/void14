...
export async function PUT(req: Request) {
  try {
    ...
    const { userInfoCategory } = await updateUserInfoCategory(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = userInfoCategoryIdSchema.parse({ id });
    const { userInfoCategory } = await deleteUserInfoCategory(validatedParams.id);
