...
export async function POST(req: Request) {
  try {
    const validatedData = insertThemeComponentStyleParams.parse(await req.json());
    const { themeComponentStyle } = await createThemeComponentStyle(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateThemeComponentStyleParams.parse(await req.json());
    const validatedParams = themeComponentStyleIdSchema.parse({ id });

    const { themeComponentStyle } = await updateThemeComponentStyle(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = themeComponentStyleIdSchema.parse({ id });
    const { themeComponentStyle } = await deleteThemeComponentStyle(validatedParams.id);
