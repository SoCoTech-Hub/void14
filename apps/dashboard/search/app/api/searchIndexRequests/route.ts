...
export async function PUT(req: Request) {
  try {
    ...
    const { searchIndexRequest } = await updateSearchIndexRequest(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = searchIndexRequestIdSchema.parse({ id });
    const { searchIndexRequest } = await deleteSearchIndexRequest(validatedParams.id);
