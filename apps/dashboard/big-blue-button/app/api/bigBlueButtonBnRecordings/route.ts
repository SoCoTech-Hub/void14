...
export async function POST(req: Request) {
  try {
    const validatedData = insertBigBlueButtonBnRecordingParams.parse(await req.json());
    const { bigBlueButtonBnRecording } = await createBigBlueButtonBnRecording(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateBigBlueButtonBnRecordingParams.parse(await req.json());
    const validatedParams = bigBlueButtonBnRecordingIdSchema.parse({ id });

    const { bigBlueButtonBnRecording } = await updateBigBlueButtonBnRecording(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = bigBlueButtonBnRecordingIdSchema.parse({ id });
    const { bigBlueButtonBnRecording } = await deleteBigBlueButtonBnRecording(validatedParams.id);
