...
export async function POST(req: Request) {
  try {
    const validatedData = insertCourseFormatOptionParams.parse(await req.json());
    const { courseFormatOption } = await createCourseFormatOption(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateCourseFormatOptionParams.parse(await req.json());
    const validatedParams = courseFormatOptionIdSchema.parse({ id });

    const { courseFormatOption } = await updateCourseFormatOption(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = courseFormatOptionIdSchema.parse({ id });
    const { courseFormatOption } = await deleteCourseFormatOption(validatedParams.id);
