...
export async function POST(req: Request) {
  try {
    const validatedData = insertToolMonitorSubscriptionParams.parse(await req.json());
    const { toolMonitorSubscription } = await createToolMonitorSubscription(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateToolMonitorSubscriptionParams.parse(await req.json());
    const validatedParams = toolMonitorSubscriptionIdSchema.parse({ id });

    const { toolMonitorSubscription } = await updateToolMonitorSubscription(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = toolMonitorSubscriptionIdSchema.parse({ id });
    const { toolMonitorSubscription } = await deleteToolMonitorSubscription(validatedParams.id);
