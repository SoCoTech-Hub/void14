...
export async function PUT(req: Request) {
  try {
    ...
    const { toolMonitorEvent } = await updateToolMonitorEvent(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = toolMonitorEventIdSchema.parse({ id });
    const { toolMonitorEvent } = await deleteToolMonitorEvent(validatedParams.id);
