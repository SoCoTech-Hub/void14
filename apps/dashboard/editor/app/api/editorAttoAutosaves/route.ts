...
export async function PUT(req: Request) {
  try {
    ...
    const { editorAttoAutosave } = await updateEditorAttoAutosave(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = editorAttoAutosaveIdSchema.parse({ id });
    const { editorAttoAutosave } = await deleteEditorAttoAutosave(validatedParams.id);
