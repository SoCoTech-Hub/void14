...
export async function POST(req: Request) {
  try {
    const validatedData = insertQuestionResponseCountParams.parse(await req.json());
    const { questionResponseCount } = await createQuestionResponseCount(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateQuestionResponseCountParams.parse(await req.json());
    const validatedParams = questionResponseCountIdSchema.parse({ id });

    const { questionResponseCount } = await updateQuestionResponseCount(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = questionResponseCountIdSchema.parse({ id });
    const { questionResponseCount } = await deleteQuestionResponseCount(validatedParams.id);
