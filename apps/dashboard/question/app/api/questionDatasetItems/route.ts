...
} from "@soco/question-api/questionDatasetItems/mutations";
import { 
...
export async function POST(req: Request) {
  try {
    const validatedData = insertQuestionDatasetItemParams.parse(await req.json());
    const { questionDatasetItem } = await createQuestionDatasetItem(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateQuestionDatasetItemParams.parse(await req.json());
    const validatedParams = questionDatasetItemIdSchema.parse({ id });

    const { questionDatasetItem } = await updateQuestionDatasetItem(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = questionDatasetItemIdSchema.parse({ id });
    const { questionDatasetItem } = await deleteQuestionDatasetItem(validatedParams.id);
