...
export async function POST(req: Request) {
  try {
    const validatedData = insertQuestionMultianswerParams.parse(await req.json());
    const { questionMultianswer } = await createQuestionMultianswer(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateQuestionMultianswerParams.parse(await req.json());
    const validatedParams = questionMultianswerIdSchema.parse({ id });

    const { questionMultianswer } = await updateQuestionMultianswer(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = questionMultianswerIdSchema.parse({ id });
    const { questionMultianswer } = await deleteQuestionMultianswer(validatedParams.id);
