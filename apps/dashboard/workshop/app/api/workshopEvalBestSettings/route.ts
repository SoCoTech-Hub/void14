...
export async function POST(req: Request) {
  try {
    const validatedData = insertWorkshopEvalBestSettingParams.parse(await req.json());
    const { workshopEvalBestSetting } = await createWorkshopEvalBestSetting(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateWorkshopEvalBestSettingParams.parse(await req.json());
    const validatedParams = workshopEvalBestSettingIdSchema.parse({ id });

    const { workshopEvalBestSetting } = await updateWorkshopEvalBestSetting(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = workshopEvalBestSettingIdSchema.parse({ id });
    const { workshopEvalBestSetting } = await deleteWorkshopEvalBestSetting(validatedParams.id);
