...
export async function PUT(req: Request) {
  try {
    ...
    const { feedbackTemplate } = await updateFeedbackTemplate(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = feedbackTemplateIdSchema.parse({ id });
    const { feedbackTemplate } = await deleteFeedbackTemplate(validatedParams.id);
