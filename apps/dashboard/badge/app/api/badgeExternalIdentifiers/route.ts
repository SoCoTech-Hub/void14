...
export async function POST(req: Request) {
  try {
    const validatedData = insertBadgeExternalIdentifierParams.parse(await req.json());
    const { badgeExternalIdentifier } = await createBadgeExternalIdentifier(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateBadgeExternalIdentifierParams.parse(await req.json());
    const validatedParams = badgeExternalIdentifierIdSchema.parse({ id });

    const { badgeExternalIdentifier } = await updateBadgeExternalIdentifier(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = badgeExternalIdentifierIdSchema.parse({ id });
    const { badgeExternalIdentifier } = await deleteBadgeExternalIdentifier(validatedParams.id);
