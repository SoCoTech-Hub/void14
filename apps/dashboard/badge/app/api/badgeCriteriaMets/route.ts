...
export async function PUT(req: Request) {
  try {
    ...
    const { badgeCriteriaMet } = await updateBadgeCriteriaMet(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = badgeCriteriaMetIdSchema.parse({ id });
    const { badgeCriteriaMet } = await deleteBadgeCriteriaMet(validatedParams.id);
