...
import { CompetencyTemplateCohort, NewCompetencyTemplateCohortParams, insertCompetencyTemplateCohortParams } from "@/lib/db/schema/competencyTemplateCohorts";
...
const CompetencyTemplateCohortForm = ({
  ...
  const { data: competencyTemplates } = trpc.competencyTemplates.getCompetencyTemplates.useQuery();
  ...
  const handleSubmit = (values: NewCompetencyTemplateCohortParams) => {
    if (editing) {
      updateCompetencyTemplateCohort({ ...values, id: competencyTemplateCohort.id });
    } else {
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className={"space-y-8"}>
          ...
          render={({ field }) => (<FormItem>
              <FormLabel>Competency Template Id</FormLabel>
                ...
                >
                  ...
                  <SelectContent>
                    {competencyTemplates?.competencyTemplates.map((competencyTemplate) => (
                      <SelectItem key={competencyTemplate.id} value={competencyTemplate.id.toString()}>
                        {competencyTemplate.id}  {/* TODO: Replace with a field from the competencyTemplate model */}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
        ...
        {editing ? (
          <Button
            type="button"
            variant={"destructive"}
            onClick={() => deleteCompetencyTemplateCohort({ id: competencyTemplateCohort.id })}
          >
