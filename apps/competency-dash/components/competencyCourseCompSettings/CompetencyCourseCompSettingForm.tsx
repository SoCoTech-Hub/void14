...
import { CompetencyCourseCompSetting, NewCompetencyCourseCompSettingParams, insertCompetencyCourseCompSettingParams } from "@/lib/db/schema/competencyCourseCompSettings";
...
const CompetencyCourseCompSettingForm = ({
  ...
  const handleSubmit = (values: NewCompetencyCourseCompSettingParams) => {
    if (editing) {
      updateCompetencyCourseCompSetting({ ...values, id: competencyCourseCompSetting.id });
    } else {
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className={"space-y-8"}>
        ...
        {editing ? (
          <Button
            type="button"
            variant={"destructive"}
            onClick={() => deleteCompetencyCourseCompSetting({ id: competencyCourseCompSetting.id })}
          >
