...
import { boolean, varchar, text, date, timestamp, pgTable } from "drizzle-orm/pg-core";
...
export const insertCompetencyTemplateSchema = createInsertSchema(competencyTemplates).omit(timestamps);
export const insertCompetencyTemplateParams = baseSchema.extend({
...
export const updateCompetencyTemplateParams = baseSchema.extend({
  visible: z.coerce.boolean(),
  dueDate: z.coerce.string().min(1)
}).omit({ 
  userId: true
...
export type NewCompetencyTemplate = z.infer<typeof insertCompetencyTemplateSchema>;
export type NewCompetencyTemplateParams = z.infer<typeof insertCompetencyTemplateParams>;
export type UpdateCompetencyTemplateParams = z.infer<typeof updateCompetencyTemplateParams>;
export type CompetencyTemplateId = z.infer<typeof competencyTemplateIdSchema>["id"];
    
// this type infers the return from getCompetencyTemplates() - meaning it will include any joins
export type CompleteCompetencyTemplate = Awaited<ReturnType<typeof getCompetencyTemplates>>["competencyTemplates"][number];
