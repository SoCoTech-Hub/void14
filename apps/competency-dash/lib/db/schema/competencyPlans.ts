...
import { varchar, text, date, boolean, timestamp, pgTable } from "drizzle-orm/pg-core";
...
export const insertCompetencyPlanSchema = createInsertSchema(competencyPlans).omit(timestamps);
export const insertCompetencyPlanParams = baseSchema.extend({
...
export const updateCompetencyPlanParams = baseSchema.extend({
  dueDate: z.coerce.string().min(1),
  status: z.coerce.boolean()
}).omit({ 
  userId: true
...
export type UpdateCompetencyPlanParams = z.infer<typeof updateCompetencyPlanParams>;
...
export type CompleteCompetencyPlan = Awaited<ReturnType<typeof getCompetencyPlans>>["competencyPlans"][number];
