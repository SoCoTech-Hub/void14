...
export const competencyTemplateComps = pgTable('competency_template_comps', {
  ...
  return {
    sortOrderIndex: uniqueIndex('sort_order_idx').on(competencyTemplateComps.sortOrder),
  }
});
...
export const insertCompetencyTemplateCompSchema = createInsertSchema(competencyTemplateComps).omit(timestamps);
export const insertCompetencyTemplateCompParams = baseSchema.extend({
...
export const updateCompetencyTemplateCompParams = baseSchema.extend({
  competencyId: z.coerce.string().min(1),
  competencyTemplateId: z.coerce.string().min(1),
  sortOrder: z.coerce.number()
}).omit({ 
...
export type NewCompetencyTemplateComp = z.infer<typeof insertCompetencyTemplateCompSchema>;
export type NewCompetencyTemplateCompParams = z.infer<typeof insertCompetencyTemplateCompParams>;
export type UpdateCompetencyTemplateCompParams = z.infer<typeof updateCompetencyTemplateCompParams>;
export type CompetencyTemplateCompId = z.infer<typeof competencyTemplateCompIdSchema>["id"];
    
// this type infers the return from getCompetencyTemplateComps() - meaning it will include any joins
export type CompleteCompetencyTemplateComp = Awaited<ReturnType<typeof getCompetencyTemplateComps>>["competencyTemplateComps"][number];
