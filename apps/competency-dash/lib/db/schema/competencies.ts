...
export const competencies = pgTable('competencies', {
  ...
  competencyFrameworkId: varchar("competency_framework_id", { length: 256 }).references(() => competencyFrameworks.id, { onDelete: "cascade" }).notNull(),
  ...
  return {
    sortOrderIndex: uniqueIndex('sort_order_idx').on(competencies.sortOrder),
  }
});
...
export const insertCompetencySchema = createInsertSchema(competencies).omit(timestamps);
export const insertCompetencyParams = baseSchema.extend({
...
export const updateCompetencyParams = baseSchema.extend({
  competencyFrameworkId: z.coerce.string().min(1),
  scaleId: z.coerce.number(),
  sortOrder: z.coerce.number()
}).omit({ 
...
export type CompleteCompetency = Awaited<ReturnType<typeof getCompetencies>>["competencies"][number];
