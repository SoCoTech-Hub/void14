...
export const competencyCourseComps = pgTable('competency_course_comps', {
  ...
  return {
    sortOrderIndex: uniqueIndex('sort_order_idx').on(competencyCourseComps.sortOrder),
  }
});
...
export const insertCompetencyCourseCompSchema = createInsertSchema(competencyCourseComps).omit(timestamps);
export const insertCompetencyCourseCompParams = baseSchema.extend({
...
export const updateCompetencyCourseCompParams = baseSchema.extend({
  competencyId: z.coerce.string().min(1),
  sortOrder: z.coerce.number()
}).omit({ 
  userId: true
...
export type NewCompetencyCourseComp = z.infer<typeof insertCompetencyCourseCompSchema>;
export type NewCompetencyCourseCompParams = z.infer<typeof insertCompetencyCourseCompParams>;
export type UpdateCompetencyCourseCompParams = z.infer<typeof updateCompetencyCourseCompParams>;
export type CompetencyCourseCompId = z.infer<typeof competencyCourseCompIdSchema>["id"];
    
// this type infers the return from getCompetencyCourseComps() - meaning it will include any joins
export type CompleteCompetencyCourseComp = Awaited<ReturnType<typeof getCompetencyCourseComps>>["competencyCourseComps"][number];
