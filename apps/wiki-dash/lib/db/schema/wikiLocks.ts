...
export const wikiLocks = pgTable('wiki_locks', {
  ...
  wikiPageId: varchar("wiki_page_id", { length: 256 }).references(() => wikiPages.id).notNull(),
...
export const insertWikiLockParams = baseSchema.extend({
...
export const updateWikiLockParams = baseSchema.extend({
  lockeDate: z.coerce.string().min(1),
  wikiPageId: z.coerce.string().min(1)
}).omit({ 
  userId: true
...
export type CompleteWikiLock = Awaited<ReturnType<typeof getWikiLocks>>["wikiLocks"][number];
