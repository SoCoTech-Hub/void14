...
export const wikiSynonyms = pgTable('wiki_synonyms', {
  id: varchar("id", { length: 191 }).primaryKey().$defaultFn(() => nanoid()),
  wikiPageId: varchar("wiki_page_id", { length: 256 }).references(() => wikiPages.id, { onDelete: "cascade" }).notNull(),
  pageSynonym: varchar("page_synonym", { length: 256 }),
  wikiSubwikiId: varchar("wiki_subwiki_id", { length: 256 }).references(() => wikiSubwikis.id, { onDelete: "cascade" }).notNull()
}, (wikiSynonyms) => {
  return {
    wikiPageIdIndex: uniqueIndex('wiki_page_id_idx').on(wikiSynonyms.wikiPageId),
  }
});
...
export const insertWikiSynonymParams = baseSchema.extend({
...
export type CompleteWikiSynonym = Awaited<ReturnType<typeof getWikiSynonyms>>["wikiSynonyms"][number];
