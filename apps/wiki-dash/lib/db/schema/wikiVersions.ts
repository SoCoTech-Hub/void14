...
export const wikiVersions = pgTable('wiki_versions', {
  ...
  return {
    wikiPageIdIndex: uniqueIndex('wiki_page_id_idx').on(wikiVersions.wikiPageId),
  }
});
...
export const insertWikiVersionSchema = createInsertSchema(wikiVersions).omit(timestamps);
export const insertWikiVersionParams = baseSchema.extend({
...
export const updateWikiVersionParams = baseSchema.extend({
  contentFormat: z.coerce.number(),
  wikiPageId: z.coerce.string().min(1),
  version: z.coerce.number()
}).omit({ 
...
export type CompleteWikiVersion = Awaited<ReturnType<typeof getWikiVersions>>["wikiVersions"][number];
