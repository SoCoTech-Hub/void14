...
export async function POST(req: Request) {
	try {
		const validatedData = insertCompetencyUserCompCourseParams.parse(
			await req.json()
		)
		const { competencyUserCompCourse } =
			await createCompetencyUserCompCourse(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateCompetencyUserCompCourseParams.parse(
			await req.json()
		)
		const validatedParams = competencyUserCompCourseIdSchema.parse({ id })

		const { competencyUserCompCourse } = await updateCompetencyUserCompCourse(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = competencyUserCompCourseIdSchema.parse({ id })
		const { competencyUserCompCourse } = await deleteCompetencyUserCompCourse(
			validatedParams.id
		)
