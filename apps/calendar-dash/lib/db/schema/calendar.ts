...
import { type getCalendar } from "@/lib/api/calendar/queries";
...
export const insertCalendarSchema = createInsertSchema(calendar).omit(timestamps);
export const insertCalendarParams = baseSchema.extend({
...
export const updateCalendarParams = baseSchema.extend({
  startDate: z.coerce.string().min(1),
  endDate: z.coerce.string().min(1)
}).omit({ 
  userId: true
...
// this type infers the return from getCalendar() - meaning it will include any joins
export type CompleteCalendar = Awaited<ReturnType<typeof getCalendar>>["calendar"][number];
