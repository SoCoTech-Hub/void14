...
export async function POST(req: Request) {
	try {
		const validatedData = insertMessageAirnotifierDeviceParams.parse(
			await req.json()
		)
		const { messageAirnotifierDevice } =
			await createMessageAirnotifierDevice(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateMessageAirnotifierDeviceParams.parse(
			await req.json()
		)
		const validatedParams = messageAirnotifierDeviceIdSchema.parse({ id })

		const { messageAirnotifierDevice } = await updateMessageAirnotifierDevice(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = messageAirnotifierDeviceIdSchema.parse({ id })
		const { messageAirnotifierDevice } = await deleteMessageAirnotifierDevice(
			validatedParams.id
		)
