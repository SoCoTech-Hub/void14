...
export const socialReactions = pgTable('social_reactions', {
  ...
  blogId: varchar("blog_id", { length: 256 }).references(() => blogs.id).notNull(),
  socialIconId: varchar("social_icon_id", { length: 256 }).references(() => socialIcons.id).notNull(),
  ...
  return {
    blogIdIndex: uniqueIndex('social_reactions_blog_id_idx').on(socialReactions.blogId),
  }
});
...
export const insertSocialReactionSchema = createInsertSchema(socialReactions).omit(timestamps);
export const insertSocialReactionParams = baseSchema.extend({
...
export const updateSocialReactionParams = baseSchema.extend({
  blogId: z.coerce.string().min(1),
  socialIconId: z.coerce.string().min(1)
}).omit({ 
  userId: true
...
export type UpdateSocialReactionParams = z.infer<typeof updateSocialReactionParams>;
...
export type CompleteSocialReaction = Awaited<ReturnType<typeof getSocialReactions>>["socialReactions"][number];
