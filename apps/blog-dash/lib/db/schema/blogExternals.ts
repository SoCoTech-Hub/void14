...
export const insertBlogExternalSchema = createInsertSchema(blogExternals).omit(timestamps);
export const insertBlogExternalParams = baseSchema.extend({
...
export const updateBlogExternalParams = baseSchema.extend({
  failedLastSync: z.coerce.boolean()
}).omit({ 
  userId: true
});
...
export type CompleteBlogExternal = Awaited<ReturnType<typeof getBlogExternals>>["blogExternals"][number];
