...
export async function POST(req: Request) {
	try {
		const validatedData = insertToolDataprivacyCtxExpiredParams.parse(
			await req.json()
		)
		const { toolDataprivacyCtxExpired } =
			await createToolDataprivacyCtxExpired(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateToolDataprivacyCtxExpiredParams.parse(
			await req.json()
		)
		const validatedParams = toolDataprivacyCtxExpiredIdSchema.parse({ id })

		const { toolDataprivacyCtxExpired } = await updateToolDataprivacyCtxExpired(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = toolDataprivacyCtxExpiredIdSchema.parse({ id })
		const { toolDataprivacyCtxExpired } = await deleteToolDataprivacyCtxExpired(
			validatedParams.id
		)
