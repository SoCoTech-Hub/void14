...
import { varchar, integer, boolean, text, timestamp, pgTable } from "drizzle-orm/pg-core";
...
export const insertForumPostSchema = createInsertSchema(forumPosts).omit(timestamps);
export const insertForumPostParams = baseSchema.extend({
...
export const updateForumPostParams = baseSchema.extend({
  charCount: z.coerce.number(),
  deleted: z.coerce.boolean(),
  mailed: z.coerce.boolean(),
  mailNow: z.coerce.number(),
...
export type CompleteForumPost = Awaited<ReturnType<typeof getForumPosts>>["forumPosts"][number];
