...
export const insertForumReadParams = baseSchema.extend({
...
export const updateForumReadParams = baseSchema.extend({
  firstRead: z.coerce.string().min(1),
  lastRead: z.coerce.string().min(1)
}).omit({ 
  userId: true
...
export type CompleteForumRead = Awaited<ReturnType<typeof getForumReads>>["forumReads"][number];
