...
export const insertForumDigestParams = baseSchema.extend({
...
export const updateForumDigestParams = baseSchema.extend({
  mailDigest: z.coerce.boolean()
}).omit({ 
  userId: true
});
...
export type CompleteForumDigest = Awaited<ReturnType<typeof getForumDigests>>["forumDigests"][number];
