...
export const dataContents = pgTable('data_contents', {
  ...
  fieldId: varchar("field_id", { length: 256 }).references(() => fields.id, { onDelete: "cascade" }).notNull(),
  dataRecordId: varchar("data_record_id", { length: 256 }).references(() => dataRecords.id, { onDelete: "cascade" }).notNull()
});
...
export const insertDataContentParams = baseSchema.extend({
...
export type CompleteDataContent = Awaited<ReturnType<typeof getDataContents>>["dataContents"][number];
