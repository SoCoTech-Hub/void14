...
export const insertRatingParams = baseSchema.extend({
...
export const updateRatingParams = baseSchema.extend({
  rating: z.coerce.number()
}).omit({ 
  userId: true
});
...
export type CompleteRating = Awaited<ReturnType<typeof getRatings>>["ratings"][number];
