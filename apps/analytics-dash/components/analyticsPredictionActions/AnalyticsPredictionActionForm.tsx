...
import { AnalyticsPredictionAction, NewAnalyticsPredictionActionParams, insertAnalyticsPredictionActionParams } from "@/lib/db/schema/analyticsPredictionActions";
...
const AnalyticsPredictionActionForm = ({
  ...
  const handleSubmit = (values: NewAnalyticsPredictionActionParams) => {
    if (editing) {
      updateAnalyticsPredictionAction({ ...values, id: analyticsPredictionAction.id });
    } else {
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className={"space-y-8"}>
        ...
        {editing ? (
          <Button
            type="button"
            variant={"destructive"}
            onClick={() => deleteAnalyticsPredictionAction({ id: analyticsPredictionAction.id })}
          >
