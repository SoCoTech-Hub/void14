...
import { AnalyticsUsedAnalysable, NewAnalyticsUsedAnalysableParams, insertAnalyticsUsedAnalysableParams } from "@/lib/db/schema/analyticsUsedAnalysables";
...
const AnalyticsUsedAnalysableForm = ({
  ...
  const handleSubmit = (values: NewAnalyticsUsedAnalysableParams) => {
    if (editing) {
      updateAnalyticsUsedAnalysable({ ...values, id: analyticsUsedAnalysable.id });
    } else {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className={"space-y-8"}>
        ...
        <FormField
          ...
          name="timeAnalysed"
          render={({ field }) => (<FormItem>
              <FormLabel>Time Analysed</FormLabel>
                <br />
              <Popover>
                <PopoverTrigger asChild>
                  <FormControl>
        ...
        {editing ? (
          <Button
            type="button"
            variant={"destructive"}
            onClick={() => deleteAnalyticsUsedAnalysable({ id: analyticsUsedAnalysable.id })}
          >
