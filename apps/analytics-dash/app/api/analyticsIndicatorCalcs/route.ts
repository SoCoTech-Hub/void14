...
export async function POST(req: Request) {
  try {
    const validatedData = insertAnalyticsIndicatorCalcParams.parse(await req.json());
    const { analyticsIndicatorCalc } = await createAnalyticsIndicatorCalc(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateAnalyticsIndicatorCalcParams.parse(await req.json());
    const validatedParams = analyticsIndicatorCalcIdSchema.parse({ id });

    const { analyticsIndicatorCalc } = await updateAnalyticsIndicatorCalc(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = analyticsIndicatorCalcIdSchema.parse({ id });
    const { analyticsIndicatorCalc } = await deleteAnalyticsIndicatorCalc(validatedParams.id);
