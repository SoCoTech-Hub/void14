...
import { boolean, text, varchar, integer, timestamp, pgTable } from "drizzle-orm/pg-core";
...
export const quizaccessSebQuizSettings = pgTable('quizaccess_seb_quiz_settings', {
  ...
  quizaccessSebTemplateId: varchar("quizaccess_seb_template_id", { length: 256 }).references(() => quizaccessSebTemplates.id).notNull(),
  ...
  return {
    quizaccessSebTemplateIdIndex: uniqueIndex('quizaccess_seb_template_id_idx').on(quizaccessSebQuizSettings.quizaccessSebTemplateId),
  }
});


// Schema for quizaccessSebQuizSettings - used to validate API requests
const baseSchema = createSelectSchema(quizaccessSebQuizSettings).omit(timestamps)

export const insertQuizaccessSebQuizSettingSchema = createInsertSchema(quizaccessSebQuizSettings).omit(timestamps);
export const insertQuizaccessSebQuizSettingParams = baseSchema.extend({
...
export const updateQuizaccessSebQuizSettingParams = baseSchema.extend({
  activateUrlFiltering: z.coerce.boolean(),
  allowReloadInExam: z.coerce.boolean(),
  allowSpellChecking: z.coerce.boolean(),
  allowUserQuitSeb: z.coerce.boolean(),
...
export type QuizaccessSebQuizSetting = typeof quizaccessSebQuizSettings.$inferSelect;
export type NewQuizaccessSebQuizSetting = z.infer<typeof insertQuizaccessSebQuizSettingSchema>;
export type NewQuizaccessSebQuizSettingParams = z.infer<typeof insertQuizaccessSebQuizSettingParams>;
export type UpdateQuizaccessSebQuizSettingParams = z.infer<typeof updateQuizaccessSebQuizSettingParams>;
export type QuizaccessSebQuizSettingId = z.infer<typeof quizaccessSebQuizSettingIdSchema>["id"];
    
// this type infers the return from getQuizaccessSebQuizSettings() - meaning it will include any joins
export type CompleteQuizaccessSebQuizSetting = Awaited<ReturnType<typeof getQuizaccessSebQuizSettings>>["quizaccessSebQuizSettings"][number];
