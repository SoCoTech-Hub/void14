...
export const customFieldFields = pgTable('custom_field_fields', {
  id: varchar("id", { length: 191 }).primaryKey().$defaultFn(() => nanoid()),
  customFieldCategoryId: varchar("custom_field_category_id", { length: 256 }).references(() => customFieldCategories.id, { onDelete: "cascade" }).notNull(),
  ...
  return {
    sortOrderIndex: uniqueIndex('sort_order_idx').on(customFieldFields.sortOrder),
  }
});
...
export const insertCustomFieldFieldSchema = createInsertSchema(customFieldFields).omit(timestamps);
export const insertCustomFieldFieldParams = baseSchema.extend({
...
export type NewCustomFieldFieldParams = z.infer<typeof insertCustomFieldFieldParams>;
export type UpdateCustomFieldFieldParams = z.infer<typeof updateCustomFieldFieldParams>;
...
export type CompleteCustomFieldField = Awaited<ReturnType<typeof getCustomFieldFields>>["customFieldFields"][number];
