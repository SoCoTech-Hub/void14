...
export const surveyAnswers = pgTable('survey_answers', {
  ...
  surveyQuestionId: varchar("survey_question_id", { length: 256 }).references(() => surveyQuestions.id).notNull(),
  surveyId: varchar("survey_id", { length: 256 }).references(() => surveys.id).notNull(),
...
export const insertSurveyAnswerSchema = createInsertSchema(surveyAnswers).omit(timestamps);
export const insertSurveyAnswerParams = baseSchema.extend({
...
export const updateSurveyAnswerParams = baseSchema.extend({
  surveyQuestionId: z.coerce.string().min(1),
  surveyId: z.coerce.string().min(1)
}).omit({ 
  userId: true
...
export type CompleteSurveyAnswer = Awaited<ReturnType<typeof getSurveyAnswers>>["surveyAnswers"][number];
