...
export const insertUserGradeParams = baseSchema.extend({
...
export const updateUserGradeParams = baseSchema.extend({
  gradeId: z.coerce.string().min(1)
}).omit({ 
  userId: true
});
...
export type CompleteUserGrade = Awaited<ReturnType<typeof getUserGrades>>["userGrades"][number];
