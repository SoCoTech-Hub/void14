...
export const insertUserSchoolParams = baseSchema.extend({
...
export const updateUserSchoolParams = baseSchema.extend({
  schoolId: z.coerce.string().min(1)
}).omit({ 
  userId: true
});
...
export type CompleteUserSchool = Awaited<ReturnType<typeof getUserSchools>>["userSchools"][number];
