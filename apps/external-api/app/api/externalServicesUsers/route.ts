...
export async function POST(req: Request) {
	try {
		const validatedData = insertExternalServicesUserParams.parse(
			await req.json()
		)
		const { externalServicesUser } =
			await createExternalServicesUser(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateExternalServicesUserParams.parse(
			await req.json()
		)
		const validatedParams = externalServicesUserIdSchema.parse({ id })

		const { externalServicesUser } = await updateExternalServicesUser(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = externalServicesUserIdSchema.parse({ id })
		const { externalServicesUser } = await deleteExternalServicesUser(
			validatedParams.id
		)
