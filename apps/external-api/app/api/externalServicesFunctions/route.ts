...
export async function POST(req: Request) {
	try {
		const validatedData = insertExternalServicesFunctionParams.parse(
			await req.json()
		)
		const { externalServicesFunction } =
			await createExternalServicesFunction(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateExternalServicesFunctionParams.parse(
			await req.json()
		)
		const validatedParams = externalServicesFunctionIdSchema.parse({ id })

		const { externalServicesFunction } = await updateExternalServicesFunction(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = externalServicesFunctionIdSchema.parse({ id })
		const { externalServicesFunction } = await deleteExternalServicesFunction(
			validatedParams.id
		)
