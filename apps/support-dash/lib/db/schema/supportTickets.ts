...
export const supportTickets = pgTable('support_tickets', {
  ...
  return {
    supportDepartmentIdIndex: uniqueIndex('support_department_id_idx').on(supportTickets.supportDepartmentId),
  }
});
...
export const insertSupportTicketSchema = createInsertSchema(supportTickets).omit(timestamps);
export const insertSupportTicketParams = baseSchema.extend({
...
export const updateSupportTicketParams = baseSchema.extend({
  open: z.coerce.boolean(),
  supportDepartmentId: z.coerce.string().min(1),
  supportTopicId: z.coerce.string().min(1),
  supportStatusId: z.coerce.string().min(1)
...
export type UpdateSupportTicketParams = z.infer<typeof updateSupportTicketParams>;
...
export type CompleteSupportTicket = Awaited<ReturnType<typeof getSupportTickets>>["supportTickets"][number];
