...
export async function PUT(req: Request) {
  try {
    ...
    const { questionTruefalse } = await updateQuestionTruefalse(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = questionTruefalseIdSchema.parse({ id });
    const { questionTruefalse } = await deleteQuestionTruefalse(validatedParams.id);
