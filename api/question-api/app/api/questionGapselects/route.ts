...
export async function PUT(req: Request) {
  try {
    ...
    const { questionGapselect } = await updateQuestionGapselect(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = questionGapselectIdSchema.parse({ id });
    const { questionGapselect } = await deleteQuestionGapselect(validatedParams.id);
