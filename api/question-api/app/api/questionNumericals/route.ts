...
export async function PUT(req: Request) {
  try {
    ...
    const { questionNumerical } = await updateQuestionNumerical(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = questionNumericalIdSchema.parse({ id });
    const { questionNumerical } = await deleteQuestionNumerical(validatedParams.id);
