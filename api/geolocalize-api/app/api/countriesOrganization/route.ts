...
export async function POST(req: Request) {
  try {
    const validatedData = insertCountriesOrganizationParams.parse(await req.json());
    const { countriesOrganization } = await createCountriesOrganization(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateCountriesOrganizationParams.parse(await req.json());
    const validatedParams = countriesOrganizationIdSchema.parse({ id });

    const { countriesOrganization } = await updateCountriesOrganization(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = countriesOrganizationIdSchema.parse({ id });
    const { countriesOrganization } = await deleteCountriesOrganization(validatedParams.id);
