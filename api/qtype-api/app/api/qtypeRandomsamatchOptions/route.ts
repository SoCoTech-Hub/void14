...
export async function POST(req: Request) {
	try {
		const validatedData = insertQtypeRandomsamatchOptionParams.parse(
			await req.json()
		)
		const { qtypeRandomsamatchOption } =
			await createQtypeRandomsamatchOption(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateQtypeRandomsamatchOptionParams.parse(
			await req.json()
		)
		const validatedParams = qtypeRandomsamatchOptionIdSchema.parse({ id })

		const { qtypeRandomsamatchOption } = await updateQtypeRandomsamatchOption(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = qtypeRandomsamatchOptionIdSchema.parse({ id })
		const { qtypeRandomsamatchOption } = await deleteQtypeRandomsamatchOption(
			validatedParams.id
		)
