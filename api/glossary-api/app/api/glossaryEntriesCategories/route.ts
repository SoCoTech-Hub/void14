...
export async function POST(req: Request) {
	try {
		const validatedData = insertGlossaryEntriesCategoryParams.parse(
			await req.json()
		)
		const { glossaryEntriesCategory } =
			await createGlossaryEntriesCategory(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateGlossaryEntriesCategoryParams.parse(
			await req.json()
		)
		const validatedParams = glossaryEntriesCategoryIdSchema.parse({ id })

		const { glossaryEntriesCategory } = await updateGlossaryEntriesCategory(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = glossaryEntriesCategoryIdSchema.parse({ id })
		const { glossaryEntriesCategory } = await deleteGlossaryEntriesCategory(
			validatedParams.id
		)
