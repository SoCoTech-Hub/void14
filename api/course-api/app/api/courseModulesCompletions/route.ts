...
export async function POST(req: Request) {
	try {
		const validatedData = insertCourseModulesCompletionParams.parse(
			await req.json()
		)
		const { courseModulesCompletion } =
			await createCourseModulesCompletion(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateCourseModulesCompletionParams.parse(
			await req.json()
		)
		const validatedParams = courseModulesCompletionIdSchema.parse({ id })

		const { courseModulesCompletion } = await updateCourseModulesCompletion(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = courseModulesCompletionIdSchema.parse({ id })
		const { courseModulesCompletion } = await deleteCourseModulesCompletion(
			validatedParams.id
		)
