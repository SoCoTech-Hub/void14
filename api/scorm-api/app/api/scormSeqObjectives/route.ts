...
export async function PUT(req: Request) {
  try {
    ...
    const { scormSeqObjective } = await updateScormSeqObjective(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = scormSeqObjectiveIdSchema.parse({ id });
    const { scormSeqObjective } = await deleteScormSeqObjective(validatedParams.id);
