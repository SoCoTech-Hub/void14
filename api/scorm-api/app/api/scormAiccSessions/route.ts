...
export async function PUT(req: Request) {
  try {
    ...
    const { scormAiccSession } = await updateScormAiccSession(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = scormAiccSessionIdSchema.parse({ id });
    const { scormAiccSession } = await deleteScormAiccSession(validatedParams.id);
