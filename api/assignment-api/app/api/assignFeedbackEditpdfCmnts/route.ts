...
export async function POST(req: Request) {
	try {
		const validatedData = insertAssignFeedbackEditpdfCmntParams.parse(
			await req.json()
		)
		const { assignFeedbackEditpdfCmnt } =
			await createAssignFeedbackEditpdfCmnt(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateAssignFeedbackEditpdfCmntParams.parse(
			await req.json()
		)
		const validatedParams = assignFeedbackEditpdfCmntIdSchema.parse({ id })

		const { assignFeedbackEditpdfCmnt } = await updateAssignFeedbackEditpdfCmnt(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = assignFeedbackEditpdfCmntIdSchema.parse({ id })
		const { assignFeedbackEditpdfCmnt } = await deleteAssignFeedbackEditpdfCmnt(
			validatedParams.id
		)
