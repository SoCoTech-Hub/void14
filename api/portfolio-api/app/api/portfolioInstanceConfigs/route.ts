...
export async function POST(req: Request) {
	try {
		const validatedData = insertPortfolioInstanceConfigParams.parse(
			await req.json()
		)
		const { portfolioInstanceConfig } =
			await createPortfolioInstanceConfig(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updatePortfolioInstanceConfigParams.parse(
			await req.json()
		)
		const validatedParams = portfolioInstanceConfigIdSchema.parse({ id })

		const { portfolioInstanceConfig } = await updatePortfolioInstanceConfig(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = portfolioInstanceConfigIdSchema.parse({ id })
		const { portfolioInstanceConfig } = await deletePortfolioInstanceConfig(
			validatedParams.id
		)
