...
export async function POST(req: Request) {
	try {
		const validatedData = insertAnalyticsUsedAnalysableParams.parse(
			await req.json()
		)
		const { analyticsUsedAnalysable } =
			await createAnalyticsUsedAnalysable(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateAnalyticsUsedAnalysableParams.parse(
			await req.json()
		)
		const validatedParams = analyticsUsedAnalysableIdSchema.parse({ id })

		const { analyticsUsedAnalysable } = await updateAnalyticsUsedAnalysable(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = analyticsUsedAnalysableIdSchema.parse({ id })
		const { analyticsUsedAnalysable } = await deleteAnalyticsUsedAnalysable(
			validatedParams.id
		)
