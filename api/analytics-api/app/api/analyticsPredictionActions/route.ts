...
export async function POST(req: Request) {
	try {
		const validatedData = insertAnalyticsPredictionActionParams.parse(
			await req.json()
		)
		const { analyticsPredictionAction } =
			await createAnalyticsPredictionAction(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateAnalyticsPredictionActionParams.parse(
			await req.json()
		)
		const validatedParams = analyticsPredictionActionIdSchema.parse({ id })

		const { analyticsPredictionAction } = await updateAnalyticsPredictionAction(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = analyticsPredictionActionIdSchema.parse({ id })
		const { analyticsPredictionAction } = await deleteAnalyticsPredictionAction(
			validatedParams.id
		)
