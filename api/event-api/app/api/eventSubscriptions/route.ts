...
export async function PUT(req: Request) {
  try {
    ...
    const { eventSubscription } = await updateEventSubscription(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = eventSubscriptionIdSchema.parse({ id });
    const { eventSubscription } = await deleteEventSubscription(validatedParams.id);
