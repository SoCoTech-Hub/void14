...
export async function POST(req: Request) {
  try {
    const validatedData = insertReportbuilderScheduleParams.parse(await req.json());
    const { reportbuilderSchedule } = await createReportbuilderSchedule(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateReportbuilderScheduleParams.parse(await req.json());
    const validatedParams = reportbuilderScheduleIdSchema.parse({ id });

    const { reportbuilderSchedule } = await updateReportbuilderSchedule(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = reportbuilderScheduleIdSchema.parse({ id });
    const { reportbuilderSchedule } = await deleteReportbuilderSchedule(validatedParams.id);
