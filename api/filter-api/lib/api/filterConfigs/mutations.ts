...
export const createFilterConfig = async (filterConfig: NewFilterConfigParams) => {
  const newFilterConfig = insertFilterConfigSchema.parse(filterConfig);
  try {
    const [f] =  await db.insert(filterConfigs).values(newFilterConfig).returning();
...
export const updateFilterConfig = async (id: FilterConfigId, filterConfig: UpdateFilterConfigParams) => {
...
export const deleteFilterConfig = async (id: FilterConfigId) => {
  const { id: filterConfigId } = filterConfigIdSchema.parse({ id });
  try {
    const [f] =  await db.delete(filterConfigs).where(eq(filterConfigs.id, filterConfigId!))
    .returning();
    return { filterConfig: f };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
