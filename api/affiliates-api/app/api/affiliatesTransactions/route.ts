...
export async function POST(req: Request) {
	try {
		const validatedData = insertAffiliatesTransactionParams.parse(
			await req.json()
		)
		const { affiliatesTransaction } =
			await createAffiliatesTransaction(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateAffiliatesTransactionParams.parse(
			await req.json()
		)
		const validatedParams = affiliatesTransactionIdSchema.parse({ id })

		const { affiliatesTransaction } = await updateAffiliatesTransaction(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = affiliatesTransactionIdSchema.parse({ id })
		const { affiliatesTransaction } = await deleteAffiliatesTransaction(
			validatedParams.id
		)
