...
export async function POST(req: Request) {
	try {
		const validatedData = insertMessageEmailMessageParams.parse(
			await req.json()
		)
		const { messageEmailMessage } =
			await createMessageEmailMessage(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateMessageEmailMessageParams.parse(
			await req.json()
		)
		const validatedParams = messageEmailMessageIdSchema.parse({ id })

		const { messageEmailMessage } = await updateMessageEmailMessage(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = messageEmailMessageIdSchema.parse({ id })
		const { messageEmailMessage } = await deleteMessageEmailMessage(
			validatedParams.id
		)
