...
export async function POST(req: Request) {
  try {
    const validatedData = insertCompetencyTemplateCohortParams.parse(await req.json());
    const { competencyTemplateCohort } = await createCompetencyTemplateCohort(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateCompetencyTemplateCohortParams.parse(await req.json());
    const validatedParams = competencyTemplateCohortIdSchema.parse({ id });

    const { competencyTemplateCohort } = await updateCompetencyTemplateCohort(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = competencyTemplateCohortIdSchema.parse({ id });
    const { competencyTemplateCohort } = await deleteCompetencyTemplateCohort(validatedParams.id);
