...
export async function POST(req: Request) {
	try {
		const validatedData = insertOauth2SystemAccountParams.parse(
			await req.json()
		)
		const { oauth2SystemAccount } =
			await createOauth2SystemAccount(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateOauth2SystemAccountParams.parse(
			await req.json()
		)
		const validatedParams = oauth2SystemAccountIdSchema.parse({ id })

		const { oauth2SystemAccount } = await updateOauth2SystemAccount(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = oauth2SystemAccountIdSchema.parse({ id })
		const { oauth2SystemAccount } = await deleteOauth2SystemAccount(
			validatedParams.id
		)
