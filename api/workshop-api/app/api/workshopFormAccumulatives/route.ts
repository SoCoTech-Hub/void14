...
export async function POST(req: Request) {
  try {
    const validatedData = insertWorkshopFormAccumulativeParams.parse(await req.json());
    const { workshopFormAccumulative } = await createWorkshopFormAccumulative(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateWorkshopFormAccumulativeParams.parse(await req.json());
    const validatedParams = workshopFormAccumulativeIdSchema.parse({ id });

    const { workshopFormAccumulative } = await updateWorkshopFormAccumulative(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = workshopFormAccumulativeIdSchema.parse({ id });
    const { workshopFormAccumulative } = await deleteWorkshopFormAccumulative(validatedParams.id);
