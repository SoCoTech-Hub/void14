...
export async function POST(req: Request) {
	try {
		const validatedData = insertWorkshopFormRubricConfigParams.parse(
			await req.json()
		)
		const { workshopFormRubricConfig } =
			await createWorkshopFormRubricConfig(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateWorkshopFormRubricConfigParams.parse(
			await req.json()
		)
		const validatedParams = workshopFormRubricConfigIdSchema.parse({ id })

		const { workshopFormRubricConfig } = await updateWorkshopFormRubricConfig(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = workshopFormRubricConfigIdSchema.parse({ id })
		const { workshopFormRubricConfig } = await deleteWorkshopFormRubricConfig(
			validatedParams.id
		)
