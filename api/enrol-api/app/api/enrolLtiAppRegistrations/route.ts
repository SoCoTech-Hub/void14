...
export async function POST(req: Request) {
  try {
    const validatedData = insertEnrolLtiAppRegistrationParams.parse(await req.json());
    const { enrolLtiAppRegistration } = await createEnrolLtiAppRegistration(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateEnrolLtiAppRegistrationParams.parse(await req.json());
    const validatedParams = enrolLtiAppRegistrationIdSchema.parse({ id });

    const { enrolLtiAppRegistration } = await updateEnrolLtiAppRegistration(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = enrolLtiAppRegistrationIdSchema.parse({ id });
    const { enrolLtiAppRegistration } = await deleteEnrolLtiAppRegistration(validatedParams.id);
