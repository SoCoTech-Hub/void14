...
export async function POST(req: Request) {
	try {
		const validatedData = insertGradingformGuideCriterionParams.parse(
			await req.json()
		)
		const { gradingformGuideCriterion } =
			await createGradingformGuideCriterion(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateGradingformGuideCriterionParams.parse(
			await req.json()
		)
		const validatedParams = gradingformGuideCriterionIdSchema.parse({ id })

		const { gradingformGuideCriterion } = await updateGradingformGuideCriterion(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = gradingformGuideCriterionIdSchema.parse({ id })
		const { gradingformGuideCriterion } = await deleteGradingformGuideCriterion(
			validatedParams.id
		)
