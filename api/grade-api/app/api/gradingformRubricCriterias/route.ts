...
export async function POST(req: Request) {
	try {
		const validatedData = insertGradingformRubricCriteriaParams.parse(
			await req.json()
		)
		const { gradingformRubricCriteria } =
			await createGradingformRubricCriteria(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateGradingformRubricCriteriaParams.parse(
			await req.json()
		)
		const validatedParams = gradingformRubricCriteriaIdSchema.parse({ id })

		const { gradingformRubricCriteria } = await updateGradingformRubricCriteria(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = gradingformRubricCriteriaIdSchema.parse({ id })
		const { gradingformRubricCriteria } = await deleteGradingformRubricCriteria(
			validatedParams.id
		)
