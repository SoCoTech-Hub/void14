...
export async function POST(req: Request) {
	try {
		const validatedData = insertBadgeBackpackOauth2Params.parse(
			await req.json()
		)
		const { badgeBackpackOauth2 } =
			await createBadgeBackpackOauth2(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateBadgeBackpackOauth2Params.parse(
			await req.json()
		)
		const validatedParams = badgeBackpackOauth2IdSchema.parse({ id })

		const { badgeBackpackOauth2 } = await updateBadgeBackpackOauth2(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = badgeBackpackOauth2IdSchema.parse({ id })
		const { badgeBackpackOauth2 } = await deleteBadgeBackpackOauth2(
			validatedParams.id
		)
