...
export async function POST(req: Request) {
  try {
    const validatedData = insertQuizaccessSebTemplateParams.parse(await req.json());
    const { quizaccessSebTemplate } = await createQuizaccessSebTemplate(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateQuizaccessSebTemplateParams.parse(await req.json());
    const validatedParams = quizaccessSebTemplateIdSchema.parse({ id });

    const { quizaccessSebTemplate } = await updateQuizaccessSebTemplate(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = quizaccessSebTemplateIdSchema.parse({ id });
    const { quizaccessSebTemplate } = await deleteQuizaccessSebTemplate(validatedParams.id);
