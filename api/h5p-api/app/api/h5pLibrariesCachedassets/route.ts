...
export async function POST(req: Request) {
  try {
    const validatedData = insertH5pLibrariesCachedassetParams.parse(await req.json());
    const { h5pLibrariesCachedasset } = await createH5pLibrariesCachedasset(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const validatedData = updateH5pLibrariesCachedassetParams.parse(await req.json());
    const validatedParams = h5pLibrariesCachedassetIdSchema.parse({ id });

    const { h5pLibrariesCachedasset } = await updateH5pLibrariesCachedasset(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = h5pLibrariesCachedassetIdSchema.parse({ id });
    const { h5pLibrariesCachedasset } = await deleteH5pLibrariesCachedasset(validatedParams.id);
