...
export const profiles = pgTable(
  "profiles",
  {
    ...
    addressId: varchar("address_id", { length: 256 })
      .references(() => addresses.id, { onDelete: "cascade" })
      .notNull(),
    genderId: varchar("gender_id", { length: 256 })
      .references(() => genders.id, { onDelete: "cascade" })
      .notNull(),
    nextOfKinId: varchar("next_of_kin_id", { length: 256 })
      .references(() => nextOfKins.id, { onDelete: "cascade" })
      .notNull(),
...
export const updateProfileParams = baseSchema
  .extend({
    dateOfBirth: z.coerce.string().min(1),
    addressId: z.coerce.string().min(1),
    genderId: z.coerce.string().min(1),
