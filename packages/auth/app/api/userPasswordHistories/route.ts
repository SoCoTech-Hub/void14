...
export async function POST(req: Request) {
	try {
		const validatedData = insertUserPasswordHistoryParams.parse(
			await req.json()
		)
		const { userPasswordHistory } =
			await createUserPasswordHistory(validatedData)
...
export async function PUT(req: Request) {
	try {
		...
		const validatedData = updateUserPasswordHistoryParams.parse(
			await req.json()
		)
		const validatedParams = userPasswordHistoryIdSchema.parse({ id })

		const { userPasswordHistory } = await updateUserPasswordHistory(
			validatedParams.id,
			validatedData
		)
...
export async function DELETE(req: Request) {
	try {
		...
		const validatedParams = userPasswordHistoryIdSchema.parse({ id })
		const { userPasswordHistory } = await deleteUserPasswordHistory(
			validatedParams.id
		)
